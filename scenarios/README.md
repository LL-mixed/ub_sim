...
- `mvp_2host_async_load_remote_10ms.yaml`: deterministic 10 ms remote-load
  overlap used by the ASYNC_LOAD two-node producer/consumer acceptance test.
...
`async_load_model` sections. The latter describes ASYNC_LOAD logical-coroutine,
...
QEMU model parameters. The section does not enable ASYNC_LOAD unless the QEMU
launcher also selects async-load mode.
