// Orchestration Function: qwen3_decode_all
...
void aicpu_orchestration_entry(const ChipStorageTaskArgs& orch_args) {
    ...
    PTO2_SCOPE() {
        uint32_t current_hidden_ci_shapes[2] = {16, 1024};
        ...
        for (int64_t layer_idx = 0; layer_idx < 28; layer_idx += 1) {
            PTO2_SCOPE() {
                ...
                uint32_t q_proj_norm_ci_shapes[2] = {16, 2048};
                ...
                uint32_t attn_out_ci_shapes[2] = {16, 2048};
                ...
                TaskOutputTensors alloc_1 = alloc_tensors(next_hidden_ci, q_proj_ci, k_proj_ci, v_proj_ci, q_proj_norm_ci, k_proj_norm_ci, attn_out_ci, all_q_padded_ci);
                const Tensor& next_hidden = alloc_1.get_ref(0);
                const Tensor& q_proj = alloc_1.get_ref(1);
                const Tensor& k_proj = alloc_1.get_ref(2);
                const Tensor& v_proj = alloc_1.get_ref(3);
                const Tensor& q_proj_norm = alloc_1.get_ref(4);
                const Tensor& k_proj_norm = alloc_1.get_ref(5);
                const Tensor& attn_out = alloc_1.get_ref(6);
                const Tensor& all_q_padded = alloc_1.get_ref(7);
                int64_t layer_off_h = (layer_idx * 1024);
                int64_t layer_off_q = (layer_idx * 2048);
                int64_t layer_off_inter = (layer_idx * 3072);
                int64_t layer_off_cache = (layer_idx * 65536);
                    ...
                    PTO2_SCOPE() {
                        ...
                        for (int64_t ob = 0; ob < 8; ob += 1) {
                            PTO2_SCOPE() {
                                ...
                                // Task 2: decode_q_proj
                                Arg params_t2;
                                params_t2.add_inout(q_proj);
                                ...
                                const Tensor& q_proj__co_l1_rv_v3 = q_proj;
                            }
                        }
                        for (int64_t ob = 0; ob < 4; ob += 1) {
                            PTO2_SCOPE() {

                                // Task 3: decode_kv_proj
                                Arg params_t3;
                                params_t3.add_inout(k_proj);
                                params_t3.add_inout(v_proj);
                                ...
                                const Tensor& k_proj__co_l1_rv_v3 = k_proj;
                                const Tensor& v_proj__co_l1_rv_v3 = v_proj;
                            }
                        }
                    }
                }
                for (int64_t b0 = 0; b0 < batch_padded; b0 += 16) {
                    PTO2_SCOPE() {
                        ...
                        // Task 4: decode_qk_norm
                        Arg params_t4;
                        params_t4.add_inout(q_proj_norm);
                        params_t4.add_input(q_proj);
                        params_t4.add_input(q_norm_w);
                        params_t4.add_inout(k_proj_norm);
                        params_t4.add_input(k_proj);
                        params_t4.add_input(k_norm_w);
                ...
                for (int64_t b = 0; b < user_batch; b += 1) {
                    PTO2_SCOPE() {
                        ...
                        TaskOutputTensors alloc_3 = alloc_tensors(attn_row_padded_ci, all_raw_scores_ci, all_exp_padded_ci, all_oi_tmp_ci, all_cur_mi_ci, all_cur_li_ci);
                        const Tensor& attn_row_padded = alloc_3.get_ref(0);
                        const Tensor& all_raw_scores = alloc_3.get_ref(1);
                        const Tensor& all_exp_padded = alloc_3.get_ref(2);
                        const Tensor& all_oi_tmp = alloc_3.get_ref(3);
                        const Tensor& all_cur_mi = alloc_3.get_ref(4);
                        const Tensor& all_cur_li = alloc_3.get_ref(5);
                        ...
                        for (int64_t ki = 0; ki < 1; ki += 1) {
                            PTO2_SCOPE() {
                                ...
                                // Task 6: decode_rope_kv_cache
                                Arg params_t6;
                                params_t6.add_inout(all_q_padded__rv_v2);
                                params_t6.add_inout(ext_k_cache_all);
                                params_t6.add_inout(ext_v_cache_all);
                                ...
                                params_t6.add_scalar(layer_off_cache);
                                rt_submit_aiv_task(6, params_t6);
                                const Tensor& all_q_padded__co_l1_rv_v6 = all_q_padded__rv_v2;
                                const Tensor& k_cache_all__co_l1_rv_v5 = ext_k_cache_all;
                                const Tensor& v_cache_all__co_l1_rv_v5 = ext_v_cache_all;
                            }
                        }
                        ...
                        for (int64_t gi = 0; gi < 8; gi += 1) {
                            PTO2_SCOPE() {
                                ...
                                uint32_t q_padded_offsets[2] = {static_cast<uint32_t>(((b * 128) + (gi * 16))), 0};
                                Tensor q_padded = all_q_padded__rv_v2.view(q_padded_shapes, q_padded_offsets);
                                for (int64_t sb = 0; sb < ((ctx_blocks + 63) / 64); sb += 1) {
                                    PTO2_SCOPE() {

                                        // Task 7: decode_qk_matmul
                                        Arg params_t7;
                                        params_t7.add_inout(all_raw_scores);
                                        params_t7.add_input(ext_block_table);
                                        params_t7.add_input(ext_k_cache_all);
                                        params_t7.add_input(q_padded);
                                        params_t7.add_scalar(sb);
                                        ...
                                        params_t7.add_scalar(gi);
                                        rt_submit_aic_task(7, params_t7);
                                        const Tensor& all_raw_scores__co_l1_rv_v3 = all_raw_scores;
                                    }
                                }
                            }
                        }
                            ...
                            PTO2_SCOPE() {
                                for (int64_t sb = 0; sb < ((ctx_blocks + 63) / 64); sb += 1) {
                                    PTO2_SCOPE() {
                                        ...
                                        params_t8.add_scalar(gi);
                                        rt_submit_aiv_task(8, params_t8);
                                        const Tensor& all_cur_li__co_l1_rv_v3 = all_cur_li;
                                        const Tensor& all_cur_mi__co_l1_rv_v3 = all_cur_mi;
                                        const Tensor& all_exp_padded__co_l1_rv_v3 = all_exp_padded;
                                    }
                                }
                            }
                        }
                        for (int64_t gi = 0; gi < 8; gi += 1) {
                            PTO2_SCOPE() {
                                int64_t kvh__ssa_v1 = gi;
                                for (int64_t sb = 0; sb < ((ctx_blocks + 63) / 64); sb += 1) {
                                    PTO2_SCOPE() {

                                        // Task 9: decode_sv_matmul
                                        Arg params_t9;
                                        params_t9.add_inout(all_oi_tmp);
                                        params_t9.add_input(ext_block_table);
                                        params_t9.add_input(all_exp_padded);
                                        params_t9.add_input(ext_v_cache_all);
                                        params_t9.add_scalar(sb);
                                        params_t9.add_scalar(ctx_blocks);
                                        params_t9.add_scalar(block_table_base);
                                        params_t9.add_scalar(kvh__ssa_v1);
                                        params_t9.add_scalar(gi);
                                        params_t9.add_scalar(layer_off_cache);
                                        rt_submit_aic_task(9, params_t9);
                                        const Tensor& all_oi_tmp__co_l1_rv_v3 = all_oi_tmp;
                                    }
                                }
                            }
                        }

                        // Task 10: decode_online_softmax
                        Arg params_t10;
                        params_t10.add_inout(attn_row_padded);
                        ...
                        // Task 11: decode_attention_writeback
                        Arg params_t11;
                        params_t11.add_inout(attn_row);
                ...
                for (int64_t b0 = 0; b0 < batch_padded; b0 += 16) {
                    PTO2_SCOPE() {
                        uint32_t resid1_tile_ci_shapes[2] = {16, 1024};
                        TensorCreateInfo resid1_tile_ci(resid1_tile_ci_shapes, 2, DataType::FLOAT32);
                        uint32_t post_norm_tile_ci_shapes[2] = {16, 1024};
                        TensorCreateInfo post_norm_tile_ci(post_norm_tile_ci_shapes, 2, DataType::BFLOAT16);
                        uint32_t mlp_tile_ci_shapes[2] = {16, 3072};
                        TensorCreateInfo mlp_tile_ci(mlp_tile_ci_shapes, 2, DataType::BFLOAT16);
                        TaskOutputTensors alloc_4 = alloc_tensors(resid1_tile_ci, post_norm_tile_ci, mlp_tile_ci);
                        const Tensor& resid1_tile = alloc_4.get_ref(0);
                        const Tensor& post_norm_tile = alloc_4.get_ref(1);
                        const Tensor& mlp_tile = alloc_4.get_ref(2);
                        ...
                        for (int64_t ob = 0; ob < 16; ob += 1) {
                            PTO2_SCOPE() {
                                ...
                                TaskOutputTensors alloc_5 = alloc_tensors(ret0__out_ci);
                                const Tensor& ret0__out = alloc_5.get_ref(0);
                                int64_t o0 = (ob * 64);

                                // Task 12: decode_out_proj
                                Arg params_t12;
                                params_t12.add_input(attn_out);
                                params_t12.add_input(ext_wo);
                                params_t12.add_inout(ret0__out);
                                params_t12.add_scalar(b0);
                                params_t12.add_scalar(layer_off_q);
                                params_t12.add_scalar(o0);
                                rt_submit_aic_task(12, params_t12);
                                const Tensor& o_acc = ret0__out;

                                // Task 13: decode_out_proj_residual
                                Arg params_t13;
                                params_t13.add_input(current_hidden__rv_v7);
                        ...
                        for (int64_t ob = 0; ob < 12; ob += 1) {
                            PTO2_SCOPE() {
                                ...
                                TaskOutputTensors alloc_6 = alloc_tensors(ret0__out_1_ci, ret0__out_2_ci);
                                const Tensor& ret0__out_1 = alloc_6.get_ref(0);
                                const Tensor& ret0__out_2 = alloc_6.get_ref(1);
                                int64_t o0__ssa_v1 = (ob * 256);

                                // Task 15: decode_gate_proj
                                Arg params_t15;
                ...
                current_hidden__rv_v7 = current_hidden__ssa_v8;
            }
        }
        for (int64_t b0 = 0; b0 < batch_padded; b0 += 16) {
            PTO2_SCOPE() {
                ...
                for (int64_t kb = 0; kb < 8; kb += 1) {
                    PTO2_SCOPE() {
                        ...
                        // Task 20: decode_copy_out
                        Arg params_t20;
                        params_t20.add_input(current_hidden__rv_v7);
                        params_t20.add_inout(ext_out);
                        params_t20.add_scalar(b0);
                        params_t20.add_scalar(k0);
