/*
 * ub_udma.c - URMA UDMA resource lifecycle app.
 *
 * Exercises the full URMA resource creation and binding pipeline
...
#define UDMA_APP_PAGE_SIZE  4096
...
{
    static const char ready_msg[] = "UDMA_APP_READY";
    static const char ack_msg[] = "UDMA_APP_READY_ACK";
    char buf[64];
...
static void *alloc_aligned_zero(size_t size)
{
    ...
    if (posix_memalign(&p, UDMA_APP_PAGE_SIZE, size) != 0 || p == NULL) {
        return NULL;
    }
    memset(p, 0, size);
    return p;
...
{
    off_t offset = (off_t)UDMA_MMAP_KERNEL_BUF * UDMA_APP_PAGE_SIZE;
    void *addr;
...
static void *map_udma_jetty_dsqe_page(int fd, uint32_t jetty_id)
{
    off_t offset = (off_t)(((uint64_t)jetty_id << MAP_INDEX_SHIFT_LOCAL) |
                           UDMA_MMAP_JETTY_DSQE_LOCAL) * UDMA_APP_PAGE_SIZE;
    void *addr = mmap(NULL, UDMA_APP_PAGE_SIZE, PROT_READ | PROT_WRITE,
...
{
    if (res->seg_buf == NULL) {
        res->seg_buf = alloc_aligned_zero(UDMA_APP_PAGE_SIZE);
        if (res->seg_buf == NULL) {
            return -ENOMEM;
        }
        res->seg_len = UDMA_APP_PAGE_SIZE;
    }
    if (res->jfc_ucmd_buf == NULL) {
        res->jfc_ucmd_buf = map_udma_kernel_buf(res->fd, UDMA_APP_PAGE_SIZE);
        if (res->jfc_ucmd_buf == NULL) {
            return -ENOMEM;
        }
        res->jfc_ucmd_len = UDMA_APP_PAGE_SIZE;
        res->jfc_ucmd_is_mmap = true;
    }
    if (res->jfc_db_buf == NULL) {
        res->jfc_db_buf = map_udma_kernel_buf(res->fd, UDMA_APP_PAGE_SIZE);
        if (res->jfc_db_buf == NULL) {
            return -ENOMEM;
        }
        res->jfc_db_len = UDMA_APP_PAGE_SIZE;
        res->jfc_db_is_mmap = true;
    }
    if (res->jfr_buf == NULL) {
        res->jfr_buf = map_udma_kernel_buf(res->fd, UDMA_APP_PAGE_SIZE);
        if (res->jfr_buf == NULL) {
            return -ENOMEM;
        }
        res->jfr_buf_len = UDMA_APP_PAGE_SIZE;
        res->jfr_buf_is_mmap = true;
    }
    if (res->jfr_idx_buf == NULL) {
        res->jfr_idx_buf = map_udma_kernel_buf(res->fd, UDMA_APP_PAGE_SIZE);
        if (res->jfr_idx_buf == NULL) {
            return -ENOMEM;
        }
        res->jfr_idx_buf_len = UDMA_APP_PAGE_SIZE;
        res->jfr_idx_buf_is_mmap = true;
    }
    if (res->jfr_db_buf == NULL) {
        res->jfr_db_buf = map_udma_kernel_buf(res->fd, UDMA_APP_PAGE_SIZE);
        if (res->jfr_db_buf == NULL) {
            return -ENOMEM;
        }
        res->jfr_db_buf_len = UDMA_APP_PAGE_SIZE;
        res->jfr_db_buf_is_mmap = true;
    }
    if (res->jfs_buf == NULL) {
        res->jfs_buf = alloc_aligned_zero(UDMA_APP_PAGE_SIZE);
        if (res->jfs_buf == NULL) {
            return -ENOMEM;
        }
    }
    if (res->jfs_db_buf == NULL) {
        res->jfs_db_buf = alloc_aligned_zero(UDMA_APP_PAGE_SIZE);
        if (res->jfs_db_buf == NULL) {
            return -ENOMEM;
        }
    }
    if (res->jetty_buf == NULL) {
        res->jetty_buf = map_udma_kernel_buf(res->fd, UDMA_APP_PAGE_SIZE);
        if (res->jetty_buf == NULL) {
            return -ENOMEM;
        }
        res->jetty_buf_len = UDMA_APP_PAGE_SIZE;
        res->jetty_buf_is_mmap = true;
    }
    if (res->jetty_db_buf == NULL) {
        res->jetty_db_buf = alloc_aligned_zero(UDMA_APP_PAGE_SIZE);
        if (res->jetty_db_buf == NULL) {
            return -ENOMEM;
        }
    }
                           ...
                           const struct sockaddr_in *peer_addr)
{
    ...
    while (now_ms() < deadline) {
        ...
        if (now >= next_send) {
            if (role == UDMA_ROLE_INITIATOR) {
                static const char req_msg[] = "UDMA_SYNC_REQ";

                if (udp_send_all(udp_fd, req_msg, strlen(req_msg), peer_addr) < 0) {
        ...
        n = recvfrom(udp_fd, sync_buf, sizeof(sync_buf) - 1, MSG_DONTWAIT,
        ...
        sync_buf[n] = '\0';
        if (role == UDMA_ROLE_INITIATOR) {
            static const char ack_msg[] = "UDMA_SYNC_ACK";

            if (strncmp(sync_buf, ack_msg, strlen(ack_msg)) == 0 ||
                (n >= (ssize_t)(strlen(ack_msg) + 2) &&
                 strncmp(sync_buf + 2, ack_msg, strlen(ack_msg)) == 0)) {
            ...
            if (strncmp(sync_buf, req_msg, strlen(req_msg)) != 0 &&
                !(n >= (ssize_t)(strlen(req_msg) + 2) &&
                  strncmp(sync_buf + 2, req_msg, strlen(req_msg)) == 0)) {
                continue;
            }

            for (i = 0; i < 10; i++) {
                if (udp_send_all(udp_fd, ack_msg, strlen(ack_msg), peer_addr) < 0) {
    ...
    fprintf(stderr, "[ub_udma] sync: timeout waiting for %s\n",
            role == UDMA_ROLE_INITIATOR ? "ACK" : "REQ");
    return -1;
}
...
static void cleanup_resources(struct udma_resources *res)
{
    if (res->fd >= 0) {
        ...
        if (res->jetty_alloc) {
            /* no explicit free jetty ioctl; resource freed by fd close */
        }
        if (res->jfs_alloc) {
            /* no explicit free jfs ioctl; resource freed by fd close */
        }
        if (res->jfr_alloc) {
            /* no explicit free jfr ioctl; resource freed by fd close */
        }
        if (res->jfc_alloc) {
            /* no explicit free jfc ioctl; resource freed by fd close */
        }
...
int main(void)
{
    ...
    }
    {
        ...
        ucmd.buf_len = UDMA_APP_PAGE_SIZE;
    ...
    }
    {
        ...
        ucmd.idx_len = UDMA_APP_PAGE_SIZE;
    ...
    }
    {
        ...
        g_res.jetty_dsqe_len = UDMA_APP_PAGE_SIZE;
