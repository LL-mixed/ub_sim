...
#ifndef OBMM_POOL_HELPERS_H
#define OBMM_POOL_HELPERS_H
...
#define OBMM_POOL_HELPERS_PORT         18560
...
#define OBMM_MSG_HELLO  1U
#define OBMM_MSG_READY  2U
...
static bool obmm_parse_ip_list(const char *csv,
                               char ips[OBMM_POOL_HELPERS_MAX_NODES][INET_ADDRSTRLEN],
                               int *count_out)
{
    char buf[512];
    char *saveptr = NULL;
    ...
    while (tok != NULL) {
        while (*tok == ' ' || *tok == '\t')
            tok++;
        if (*tok != '\0') {
            if (count >= OBMM_POOL_HELPERS_MAX_NODES)
                return false;
...
static bool obmm_resolve_nodes(char local_ip[INET_ADDRSTRLEN],
                               char ips[OBMM_POOL_HELPERS_MAX_NODES][INET_ADDRSTRLEN],
                               int *node_count_out, int *local_idx_out)
{
    char csv[512];
    int local_idx, node_count, i;
    if (!obmm_env_or_cmdline("LINQU_UB_LOCAL_IP", "linqu_ipourma_ipv4",
                             local_ip, INET_ADDRSTRLEN))
        return false;
    if (!obmm_env_or_cmdline("LINQU_UB_ALL_IPS", "linqu_ipourma_all_ipv4",
                             csv, sizeof(csv))) {
        int max_nodes = 2;
        char count_buf[16];
        if (obmm_env_or_cmdline("LINQU_UB_NODE_COUNT", "linqu_ipourma_node_count",
                                count_buf, sizeof(count_buf))) {
            max_nodes = atoi(count_buf);
            if (max_nodes < 2 || max_nodes > OBMM_POOL_HELPERS_MAX_NODES)
                return false;
        }
        csv[0] = '\0';
        for (i = 1; i <= max_nodes; i++) {
            ...
            if (csv[0] != '\0')
                snprintf(csv + strlen(csv), sizeof(csv) - strlen(csv), ",");
            snprintf(csv + strlen(csv), sizeof(csv) - strlen(csv), "%s", ip);
        }
    }
    if (!obmm_parse_ip_list(csv, ips, &node_count))
        return false;
    local_idx = -1;
...
static int obmm_create_udp(const char *ifname)
{
    ...
    bind_addr.sin_port = htons(OBMM_POOL_HELPERS_PORT);
...
static int obmm_send_udp(int sockfd, const struct sockaddr_in *peer,
...
static ssize_t obmm_recv_udp(int sockfd, void *buf, size_t len,
...
static void obmm_init_pool_msg(struct obmm_helpers_pool_msg *msg,
                               uint16_t type, int src_idx, int dst_idx)
{
    memset(msg, 0, sizeof(*msg));
    msg->magic = OBMM_POOL_HELPERS_MAGIC;
    msg->version = OBMM_POOL_HELPERS_VERSION;
    msg->type = type;
...
static uint64_t obmm_parse_export_size(void)
{
    ...
    bytes = obmm_align_up_u64(bytes, OBMM_POOL_HELPERS_IMPORT_ALIGN);
    return bytes;
}

#endif /* OBMM_POOL_HELPERS_H */
