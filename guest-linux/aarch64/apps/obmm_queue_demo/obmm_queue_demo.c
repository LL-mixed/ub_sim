 ...
 *   after queues:      TX arena owned by this exporter
 */

static uint64_t layout_queue_offset(int peer_idx, int node_count)
{
    /* directory follows header, then queues start after directory */
...
{
    uint64_t queues_end = layout_queue_offset(node_count - 2, node_count) +
                          obmm_queue_region_size(g_queue_depth);
...
static int validate_export_layout(int node_count)
{
    ...
    uint64_t queues_end, arena_base, arena_size;

    if (node_count < 2 || node_count > MAX_NODES)
        return -1;
    if (!checked_add_u64(peer_count, 1, &dir_entries) ||
        !checked_mul_u64(dir_entries, sizeof(struct obmm_region_dirent),
                         &dir_size) ||
    ...
    if (!checked_mul_u64(peer_count, queue_sz, &queues_size) ||
        ...
        return -1;
    arena_base = obmm_align_up_u64(queues_end, 64);
...
static int init_export_layout(void *base, int node_id, int node_count)
{
    ...
    hdr->directory_count = (uint32_t)(peer_count + 1);
...
static int resolve_peer_layout(struct node_slot *slot, int local_idx)
{
    ...
    if (hdr->directory_offset < sizeof(*hdr) ||
        ...
        fprintf(stderr, TAG " peer directory metadata invalid owner=%d\n",
                slot->owner_idx + 1);
        return -1;
    }
