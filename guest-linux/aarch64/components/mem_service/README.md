...
- `apps/mem_service/ops-certification-policy.txt` is the checked-in manifest
  ...
  ops-certification-verify --evidence-file <path>` validates that external
  evidence file, and `ops-certification-evidence-fixtures` covers the local
  positive/negative parser gate.
...
- Service API: versioned wire envelope, stable operation IDs, stable error
  ...
  file verified by `ops-certification-verify --evidence-file`.
