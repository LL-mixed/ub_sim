...
- `apps/mem_service/release-manifest.txt` freezes the current minimal release
  ...
  wire/schema versions, wire schema manifest checksum, operation IDs, and
  status IDs.
...
- `apps/mem_service` builds `/bin/linqu_mem_service` for core smoke/self-test,
  ...
  `compat-old-new-matrix`, `compat-old-new-fixtures`, `api-abi-policy`,
  `api-abi-fixtures`, `release-manifest`, and `release-fixtures`, and
...
- `apps/mem_service` also exposes `make install-smoke DESTDIR=<dir>
  ...
  schema manifest, config schema/example, and deployment manifest into a
  package-like layout.
...
- Service API: versioned wire envelope, stable operation IDs, stable error
  ...
  field, and selector counts. This is still a text key/value schema manifest,
  not a cross-version binary typed schema.
...
- Release/deployment: the current `release-manifest`, `wire-schema`,
  ...
  manifest, API/ABI policy artifact, compatibility matrix, v1 compatibility baseline, old/new
  ...
  `metrics_listen` config, `/metrics` scrape path contract, collector scrape
  contract, TCP metrics listener contract, service-manager lifecycle contract,
  durable catalog layout contract, and explicit client retry policy.
  They are not yet a full package, real systemd environment smoke,
  upgrade/rollback smoke, production collector/alert integration gate, or
  old-server runtime-binary compatibility bundle.
