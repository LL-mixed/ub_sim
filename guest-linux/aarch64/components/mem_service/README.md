...
- `apps/mem_service/upgrade-rollback-policy.txt` is the checked-in manifest
  ...
  gates, and old-server runtime-binary not-certified status for release diffing
...
- Service API: versioned wire envelope, stable operation IDs, stable error
  ...
  `upgrade-rollback-policy` freezes the current-version-only admission rule and
  required release gates; it does not certify old-server runtime binaries or
