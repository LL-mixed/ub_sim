...
- `apps/mem_service/ops-certification-policy.txt` is the checked-in manifest
  ...
  ops-certification-verify --evidence-file <path>` validates that evidence, and
  `ops-certification-evidence-fixtures` covers the local positive/negative
  parser gate.
...
- Service API: versioned wire envelope, stable operation IDs, stable error
  ...
  file generated by `ops-certification-generate-evidence` and verified by
