...
start_node() {
  ...
  env \
    ...
    "$QEMU_BIN" \
      ...
      -append "console=ttyAMA0 rdinit=/bin/run_demo obmm_gsva_demo linqu_urma_dp_role=${node_name} linqu_node_idx=${node_idx} obmm_gsva_mode=matrix obmm_gsva_base=${GSVA_MATRIX_BASE} obmm_gsva_size=${GSVA_MATRIX_SLICE_SIZE} obmm_gsva_node_count=${GSVA_MATRIX_NODE_COUNT} ${APPEND_EXTRA}" \
...
validate_node_logs() {
  ...
  for node_name in "${NODE_NAMES[@]}"; do
    ...
    route_count="$(grep -Ec 'GVA_S3_MAP .*pte_offset=0 .*address_profile=2' "$qemu_log" || true)"
    if (( route_count < GSVA_MATRIX_NODE_COUNT - 1 )); then
      echo "$LOG_PREFIX FAIL: $node_name QEMU log has too few GSVA routes: $route_count" >&2
      return 1
    fi
