...
start_node() {
  ...
  env \
    ...
    "$QEMU_BIN" \
      ...
      -append "console=ttyAMA0 rdinit=/bin/run_demo obmm_pool linqu_urma_dp_role=${role} linqu_node_idx=${node_idx} obmm_pool_local_ip=${local_ip} obmm_pool_all_ips=${OBMM_POOL_ALL_IPS} obmm_pool_node_count=2 obmm_pool_export_size_mb=${OBMM_POOL_EXPORT_SIZE_MB} obmm_pool_import_cache_mode=${OBMM_IMPORT_CACHE_MODE} obmm_pool_stress_iters=${OBMM_POOL_STRESS_ITERS} ${APPEND_EXTRA}" \
      >"$qemu_log" 2>&1 &
  echo $! > "$pid_file"
}
...
validate_node_log() {
  ...
  assert_log_has "$log_file" "\\[run_demo\\] run linqu_ub_obmm_pool" "$node_name binary" || return 1
...
while (( SECONDS < deadline )); do
  ...
  if grep -qE '\[ub_obmm_pool\] fail|\[run_demo\] linqu_ub_obmm_pool failed|Kernel panic - not syncing|Call trace:' "$NODEA_GUEST_LOG" "$NODEB_GUEST_LOG" 2>/dev/null; then
