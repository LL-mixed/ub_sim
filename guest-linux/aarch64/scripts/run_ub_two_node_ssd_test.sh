...
# SSD Test - Two Node
#
# Runs ssd_test on a 2-node QEMU setup.
...
NODEA_PID_FILE="$OUT_DIR/ub_nodeA.ssd_test.${RUN_ID}.pid"
NODEB_PID_FILE="$OUT_DIR/ub_nodeB.ssd_test.${RUN_ID}.pid"
...
start_node() {
  ...
  env \
    ...
    "$QEMU_BIN" \
      ...
      -append "console=ttyAMA0 rdinit=/bin/run_demo linqu_ssd_test=1 linqu_urma_dp_role=${role} linqu_node_idx=${node_idx} ${APPEND_EXTRA}" \
      >"$qemu_log" 2>&1 &
  echo $! > "$pid_file"
}
...
validate_ssd_logs() {
  if ! grep -q 'UB_SSD: created' "$NODEA_QEMU_LOG"; then
    echo "[ssd_test] FAIL: SSD device not created on nodeA" >&2
    return 1
  fi
  if ! grep -q 'UB_SSD: created' "$NODEB_QEMU_LOG"; then
    echo "[ssd_test] FAIL: SSD device not created on nodeB" >&2
    return 1
  fi
  return 0
}

echo "[ssd_test] run_id=$RUN_ID"
echo "[ssd_test] starting nodeA and nodeB..."
...
echo "[ssd_test] waiting for FM links..."
if ! wait_for_fm_links_ready "$NODEA_QEMU_LOG" "$NODEB_QEMU_LOG" "$LINK_WAIT_SECS"; then
  echo "[ssd_test] FAIL: FM links not ready" >&2
  exit 1
fi
echo "[ssd_test] FM links ready"

echo "[ssd_test] waiting for test completion (timeout ${RUN_SECS}s)..."
...
while (( SECONDS < deadline )); do
  if [[ -f "$NODEA_GUEST_LOG" ]] && grep -q 'verdict=PASS' "$NODEA_GUEST_LOG" && \
    ...
    sleep 0.5
    validate_ssd_logs
    echo "[ssd_test] PASS: both nodes completed"
    exit 0
  fi
  if grep -qE 'verdict=FAIL|Kernel panic - not syncing|Call trace:' "$NODEA_GUEST_LOG" "$NODEB_GUEST_LOG" 2>/dev/null; then
    echo "[ssd_test] FAIL: guest reported failure" >&2
    exit 1
  fi
  sleep 0.5
done

echo "[ssd_test] FAIL: timeout waiting for completion" >&2
exit 1
