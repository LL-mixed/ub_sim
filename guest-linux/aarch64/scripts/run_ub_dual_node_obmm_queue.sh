...
SHARED_DIR="${UB_FM_SHARED_DIR:-/tmp/ub-qemu-links-obmm-queue}"
...
RUN_ID="${RUN_ID:-$(date +%Y-%m-%d_%H-%M-%S)_obmm_queue_${RANDOM}}"
...
OBMM_QUEUE_MODE="${OBMM_QUEUE_MODE:-fullmesh}"
...
NODEA_PID_FILE="$OUT_DIR/ub_nodeA.obmm_queue.${RUN_ID}.pid"
NODEB_PID_FILE="$OUT_DIR/ub_nodeB.obmm_queue.${RUN_ID}.pid"
...
start_node() {
  ...
  env \
    ...
    "$QEMU_BIN" \
      ...
      -append "console=ttyAMA0 rdinit=/bin/run_app linqu_obmm_queue=1 obmm_queue_local_ip=${local_ip} obmm_queue_all_ips=${OBMM_QUEUE_ALL_IPS} obmm_queue_node_count=2 obmm_queue_export_size_mb=${OBMM_POOL_EXPORT_SIZE_MB} obmm_queue_depth=${OBMM_QUEUE_DEPTH} obmm_queue_bootstrap=${OBMM_BOOTSTRAP} obmm_queue_bootstrap_session=${RUN_ID} obmm_queue_mode=${OBMM_QUEUE_MODE} obmm_queue_import_cache_mode=${OBMM_IMPORT_CACHE_MODE} linqu_urma_dp_role=${role} linqu_node_idx=${node_idx} ${APPEND_EXTRA}" \
      >"$qemu_log" 2>&1 &
  echo $! > "$pid_file"
}
...
assert_log_has() {
  ...
  if ! grep -qE "$pattern" "$file"; then
    echo "[obmm-queue] FAIL: missing $label in $file" >&2
    return 1
  fi
}
...
assert_log_absent() {
  ...
  local label="$3"
  if grep -qE "$pattern" "$file"; then
    echo "[obmm-queue] FAIL: unexpected $label in $file" >&2
    return 1
  fi
}
...
validate_node_log() {
  ...
  assert_log_has "$log_file" "\\[obmm_queue\\] pool ready -> ok nodes=2" "$node_name pool" || return 1
  assert_log_has "$log_file" "\\[obmm_queue\\] rounds -> ok count=2" "$node_name rounds" || return 1
  assert_log_has "$log_file" "\\[obmm_queue\\] queue stress -> ok passes=2 depth=${OBMM_QUEUE_DEPTH}" "$node_name stress" || return 1
  assert_log_has "$log_file" "\\[obmm_queue\\] pass" "$node_name pass" || return 1
  assert_log_absent "$log_file" "\\[obmm_queue\\] .*fail" "$node_name fail" || return 1
...
echo "[obmm-queue] waiting for FM links..."
if ! wait_for_fm_links_ready "$NODEA_QEMU_LOG" "$NODEB_QEMU_LOG" "$LINK_WAIT_SECS"; then
  echo "[obmm-queue] FAIL: FM links not ready" >&2
  exit 1
fi
echo "[obmm-queue] FM links ready"

echo "[obmm-queue] waiting for app completion (timeout ${RUN_SECS}s)..."
deadline=$((SECONDS + RUN_SECS))
while (( SECONDS < deadline )); do
  if [[ -f "$NODEA_GUEST_LOG" ]] && grep -qE '\[obmm_queue\] pass' "$NODEA_GUEST_LOG" && \
     [[ -f "$NODEB_GUEST_LOG" ]] && grep -qE '\[obmm_queue\] pass' "$NODEB_GUEST_LOG"; then
    validate_node_log nodeA "$NODEA_GUEST_LOG"
    validate_node_log nodeB "$NODEB_GUEST_LOG"
    echo "[obmm-queue] PASS: both nodes completed"
    echo "[obmm-queue] nodeA:"
    grep '\[obmm_queue\]' "$NODEA_GUEST_LOG" | tail -8
    echo "[obmm-queue] nodeB:"
...
echo "[obmm-queue] FAIL: timeout waiting for completion" >&2
exit 1
