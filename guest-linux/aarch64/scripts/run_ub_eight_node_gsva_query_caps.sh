...
REPORT_FILE="${REPORT_FILE:-$OUT_DIR/eight_node_gsva_query_caps.latest.txt}"
TRACE_FILE="${TRACE_FILE:-$OUT_DIR/eight_node_gsva_query_caps.trace.latest.txt}"
RUN_ID_BASE="${RUN_ID:-$(date +%Y-%m-%d_%H-%M-%S)_gsva_query8_${RANDOM}}"
...
send_query_cmd() {
  ...
  payload+=$'/bin/linqu_ub_gsva_query --caps\n'
...
validate_node_log() {
  ...
  assert_log_has_since "$log_file" "$start_line" "\\[gsva_query\\] GSVA_QUERY_CAPS" \
    "$node_id query caps" || return 1
  assert_log_has_since "$log_file" "$start_line" "caps:.*STRICT_ADDRESS_IDENTITY" \
    "$node_id strict identity cap" || return 1
  assert_log_has_since "$log_file" "$start_line" "max_nodes:[[:space:]]+[1-9][0-9]*" \
    "$node_id max nodes" || return 1
  assert_log_has_since "$log_file" "$start_line" "supported_policies:[[:space:]]+0x[0-9a-f]+" \
    "$node_id policies" || return 1
  assert_log_has_since "$log_file" "$start_line" "verdict=PASS" \
    "$node_id pass" || return 1
  assert_log_absent_since "$log_file" "$start_line" "verdict=FAIL|Kernel panic - not syncing|Call trace:" \
...
run_query_app() {
  ...
  for node_id in "${NODE_IDS[@]}"; do
    ...
    start_marker="GSVA_QUERY_CAPS_${node_id}_START"
    trace "start query caps on $node_id serial=$serial_endpoint"
    send_query_cmd "$serial_endpoint" "$start_marker"
  done
  ...
  for node_id in "${NODE_IDS[@]}"; do
    ...
    wait_for_log_pass_or_fail_since "$guest_log" "${START_LINES[$node_id]}" \
      "verdict=PASS" "verdict=FAIL|Kernel panic - not syncing|Call trace:" \
      "$APP_WAIT_SECS" || rc=$?
    if [[ "$rc" -ne 0 ]]; then
      trace "FAIL: gsva query did not pass on $node_id rc=$rc"
      return 1
    fi
  done
