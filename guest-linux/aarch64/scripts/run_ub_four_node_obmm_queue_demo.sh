...
prepare_environment() {
  ...
  mkdir -p "$RUN_DIR"
  : > "$TRACE_FILE"
  trace "prepare: launch headless env run_id=$RUN_ID_BASE qemu_mem=$QEMU_MEM qemu_smp=$QEMU_SMP export_size_mb=$OBMM_POOL_EXPORT_SIZE_MB queue_depth=$OBMM_QUEUE_DEPTH"
...
validate_node_log() {
  ...
  assert_log_has "$log_file" "\\[obmm_queue_demo\\] bootstrap exchange -> ok count=4" \
