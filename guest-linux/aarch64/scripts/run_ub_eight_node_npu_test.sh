...
REPORT_FILE="${REPORT_FILE:-$OUT_DIR/eight_node_npu_test.latest.txt}"
TRACE_FILE="${TRACE_FILE:-$OUT_DIR/eight_node_npu_test.trace.latest.txt}"
RUN_ID_BASE="${RUN_ID:-$(date +%Y-%m-%d_%H-%M-%S)_npu8_${RANDOM}}"
...
QEMU_SMP="${QEMU_SMP:-4}"
PORT_BASE_START="${PORT_BASE_START:-$((53800 + (RANDOM % 300)))}"
...
validate_node_log() {
  ...
  assert_log_has "$qemu_log" "UB_NPU: created" "$node_id NPU device created" || return 1
  assert_log_has_since "$guest_log" "$start_line" "\\[npu_test\\] NPU test suite" \
    "$node_id suite start" || return 1
  assert_log_has_since "$guest_log" "$start_line" "\\[npu_test\\] TEST: command echo" \
    "$node_id command echo" || return 1
  assert_log_has_since "$guest_log" "$start_line" "\\[npu_test\\] TEST: bad opcode rejection" \
    "$node_id bad opcode" || return 1
  assert_log_has_since "$guest_log" "$start_line" "\\[npu_test\\] verdict=PASS" \
    "$node_id pass" || return 1
  assert_log_absent_since "$guest_log" "$start_line" "\\[npu_test\\] verdict=FAIL|Kernel panic - not syncing|Call trace:" \
...
run_npu_app() {
  ...
  for node_id in "${NODE_IDS[@]}"; do
    ...
    start_marker="NPU_TEST_${node_id}_START"
    trace "start NPU test on $node_id serial=$serial_endpoint"
    send_npu_cmd "$serial_endpoint" "$start_marker"
  done
  ...
  for node_id in "${NODE_IDS[@]}"; do
    ...
    wait_for_log_pass_or_fail_since "$guest_log" "${START_LINES[$node_id]}" \
      "\\[npu_test\\] verdict=PASS" "\\[npu_test\\] verdict=FAIL|Kernel panic - not syncing|Call trace:" \
      "$APP_WAIT_SECS" || rc=$?
    if [[ "$rc" -ne 0 ]]; then
      trace "FAIL: NPU test did not pass on $node_id rc=$rc"
      return 1
    fi
  done
