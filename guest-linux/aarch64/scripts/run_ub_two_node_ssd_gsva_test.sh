...
# SSD GSVA Test - Two Node
#
# Runs ssd_gsva_test on a 2-node QEMU setup to validate SSD
# BLOCK_WRITE/BLOCK_READ round-trip via GSVA segments.
...
NODEA_PID_FILE="$OUT_DIR/ub_nodeA.ssd_gsva_test.${RUN_ID}.pid"
NODEB_PID_FILE="$OUT_DIR/ub_nodeB.ssd_gsva_test.${RUN_ID}.pid"
...
start_node() {
  ...
  env \
    ...
    "$QEMU_BIN" \
      ...
      -append "console=ttyAMA0 rdinit=/bin/run_demo linqu_ssd_gsva_test=1 linqu_urma_dp_role=${role} linqu_node_idx=${node_idx} ${APPEND_EXTRA}" \
      >"$qemu_log" 2>&1 &
  echo $! > "$pid_file"
}
...
echo "[ssd_gsva_test] starting nodeA and nodeB..."
...
echo "[ssd_gsva_test] waiting for FM links..."
if ! wait_for_fm_links_ready "$NODEA_QEMU_LOG" "$NODEB_QEMU_LOG" "$LINK_WAIT_SECS"; then
  echo "[ssd_gsva_test] FAIL: FM links not ready" >&2
  exit 1
fi
echo "[ssd_gsva_test] FM links ready"

echo "[ssd_gsva_test] waiting for test completion (timeout ${RUN_SECS}s)..."
...
while (( SECONDS < deadline )); do
  if [[ -f "$NODEA_GUEST_LOG" ]] && grep -q 'verdict=PASS' "$NODEA_GUEST_LOG" && \
    ...
    sleep 0.5
    validate_ssd_gsva_logs
    echo "[ssd_gsva_test] PASS: both nodes completed"
    exit 0
  fi
  if grep -q 'verdict=SKIP' "$NODEA_GUEST_LOG" "$NODEB_GUEST_LOG" 2>/dev/null; then
    echo "[ssd_gsva_test] FAIL: guest skipped test" >&2
    exit 1
  fi
  if grep -qE 'verdict=FAIL|Kernel panic - not syncing|Call trace:' "$NODEA_GUEST_LOG" "$NODEB_GUEST_LOG" 2>/dev/null; then
    echo "[ssd_gsva_test] FAIL: guest reported failure" >&2
    exit 1
  fi
  sleep 0.5
done

echo "[ssd_gsva_test] FAIL: timeout waiting for completion" >&2
exit 1
