...
# SSD GSVA Test - Four Node
#
# Runs ssd_gsva_test on a 4-node QEMU setup.
...
start_node() {
  ...
  env \
    ...
    "$QEMU_BIN" \
      ...
      -append "console=ttyAMA0 rdinit=/bin/run_demo linqu_ssd_gsva_test=1 linqu_urma_dp_role=${role} linqu_node_idx=${node_idx} linqu_node_count=4 ${APPEND_EXTRA}" \
      >"$qemu_log" 2>&1 &
  echo $! > "$pid_file"
}
...
validate_ssd_gsva_logs() {
  ...
  for node_suffix in "${NODES[@]}"; do
    validate_ub_gsva_trace_logs "[ssd_gsva_test_4]" ssd "node${node_suffix}" \
      "${QEMU_LOGS[$node_suffix]}" "${GUEST_LOGS[$node_suffix]}" || rc=1
    validate_ub_gsva_peer_matrix "[ssd_gsva_test_4]" "node${node_suffix}" \
...
echo "[ssd_gsva_test_4] run_id=$RUN_ID"
echo "[ssd_gsva_test_4] starting 4 nodes..."
...
echo "[ssd_gsva_test_4] waiting for FM links..."
if ! wait_all_links_ready "$LINK_WAIT_SECS"; then
  echo "[ssd_gsva_test_4] FAIL: FM links not ready" >&2
  exit 1
fi
echo "[ssd_gsva_test_4] FM links ready"

echo "[ssd_gsva_test_4] waiting for test completion (timeout ${RUN_SECS}s)..."
...
while (( SECONDS < deadline )); do
  ...
  if [[ "$all_pass" == "true" ]]; then
    ...
    sleep 0.5
    validate_ssd_gsva_logs
    echo "[ssd_gsva_test_4] PASS: all nodes completed"
    exit 0
  fi
  for node_suffix in "${NODES[@]}"; do
    local guest_log="${GUEST_LOGS[$node_suffix]}"
    if grep -qE 'verdict=FAIL|Kernel panic - not syncing|Call trace:' "$guest_log" 2>/dev/null; then
      echo "[ssd_gsva_test_4] FAIL: node${node_suffix} reported failure" >&2
      exit 1
    fi
  done
  sleep 0.5
...
echo "[ssd_gsva_test_4] FAIL: timeout waiting for completion" >&2
exit 1
