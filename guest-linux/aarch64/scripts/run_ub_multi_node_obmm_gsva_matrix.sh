...
OBMM_GSVA_MATRIX_NODE_COUNT="${OBMM_GSVA_MATRIX_NODE_COUNT:-${GSVA_MATRIX_NODE_COUNT:-4}}"
case "$OBMM_GSVA_MATRIX_NODE_COUNT" in
  4)
  ...
  8)
    ...
    ;;
  *)
    echo "[obmm-gsva-matrix] FAIL: OBMM_GSVA_MATRIX_NODE_COUNT must be 4 or 8" >&2
    exit 1
    ;;
esac
...
RUN_ID="${RUN_ID:-$(date +%Y-%m-%d_%H-%M-%S)_obmm_gsva_matrix${OBMM_GSVA_MATRIX_NODE_COUNT}_${RANDOM}}"
OBMM_GSVA_MATRIX_BASE="${OBMM_GSVA_MATRIX_BASE:-${GSVA_MATRIX_BASE:-0x700000000000}}"
OBMM_GSVA_MATRIX_SLICE_SIZE="${OBMM_GSVA_MATRIX_SLICE_SIZE:-${GSVA_MATRIX_SLICE_SIZE:-0x400000}}"
GSVA_MODE="${GSVA_MODE:-arm_mmu}"
GSVA_STRICT="${GSVA_STRICT:-1}"
LOG_PREFIX="[obmm-gsva-matrix${OBMM_GSVA_MATRIX_NODE_COUNT}]"
...
pid_file_for() {
  echo "$OUT_DIR/ub_$1.obmm_gsva_matrix${OBMM_GSVA_MATRIX_NODE_COUNT}.${RUN_ID}.pid"
}
...
wait_for_fm_endpoints() {
  ...
  local expected=$((OBMM_GSVA_MATRIX_NODE_COUNT * (OBMM_GSVA_MATRIX_NODE_COUNT - 1)))
...
validate_node_logs() {
  ...
  for node_name in "${NODE_NAMES[@]}"; do
    ...
    slice_base="$((OBMM_GSVA_MATRIX_BASE + node_idx * OBMM_GSVA_MATRIX_SLICE_SIZE))"
    expected_node0="$(matrix_value 0 "$node_idx")"
    expected_last="$(matrix_value "$((OBMM_GSVA_MATRIX_NODE_COUNT - 1))" "$node_idx")"
    ...
    if ! grep -q "\[obmm_gsva\] result=done mode=matrix node=${node_idx} node_count=${OBMM_GSVA_MATRIX_NODE_COUNT}" "$guest_log"; then
    ...
    if ! validate_mp_table_log "$node_name" "$qemu_log" "$((OBMM_GSVA_MATRIX_NODE_COUNT - 1))"; then
      return 1
    fi
...
echo "$LOG_PREFIX run_id=$RUN_ID base=$OBMM_GSVA_MATRIX_BASE slice_size=$OBMM_GSVA_MATRIX_SLICE_SIZE"
echo "$LOG_PREFIX topology=$TOPOLOGY_FILE qemu_mem=$QEMU_MEM qemu_smp=$QEMU_SMP port_num=$PORT_NUM"
echo "$LOG_PREFIX starting ${OBMM_GSVA_MATRIX_NODE_COUNT} nodes..."
...
while (( SECONDS < deadline )); do
  ...
  if (( done_count == OBMM_GSVA_MATRIX_NODE_COUNT )); then
    validate_node_logs
  ...
  if (( fail_count > 0 )); then
    echo "$LOG_PREFIX FAIL: matrix app reported failure" >&2
    for node_name in "${NODE_NAMES[@]}"; do
