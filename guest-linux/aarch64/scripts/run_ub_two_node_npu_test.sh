...
# NPU Test - Two Node
#
# Runs npu_test on a 2-node QEMU setup.
...
NODEA_PID_FILE="$OUT_DIR/ub_nodeA.npu_test.${RUN_ID}.pid"
NODEB_PID_FILE="$OUT_DIR/ub_nodeB.npu_test.${RUN_ID}.pid"
...
start_node() {
  ...
  env \
    ...
    UB_SIM_EXPERIMENTAL_FEATURES=npu \
    "$QEMU_BIN" \
      ...
      -append "console=ttyAMA0 rdinit=/bin/run_app linqu_npu_test=1 linqu_urma_dp_role=${role} linqu_node_idx=${node_idx} ${APPEND_EXTRA}" \
      >"$qemu_log" 2>&1 &
  echo $! > "$pid_file"
}
...
validate_npu_logs() {
  if ! grep -q 'UB_NPU: created' "$NODEA_QEMU_LOG"; then
    echo "[npu_test] FAIL: NPU device not created on nodeA" >&2
    return 1
  fi
  if ! grep -q 'UB_NPU: created' "$NODEB_QEMU_LOG"; then
    echo "[npu_test] FAIL: NPU device not created on nodeB" >&2
    return 1
  fi
  return 0
}

echo "[npu_test] run_id=$RUN_ID"
echo "[npu_test] starting nodeA and nodeB..."
...
echo "[npu_test] waiting for FM links..."
if ! wait_for_fm_links_ready "$NODEA_QEMU_LOG" "$NODEB_QEMU_LOG" "$LINK_WAIT_SECS"; then
  echo "[npu_test] FAIL: FM links not ready" >&2
  exit 1
fi
echo "[npu_test] FM links ready"

echo "[npu_test] waiting for test completion (timeout ${RUN_SECS}s)..."
...
while (( SECONDS < deadline )); do
  if [[ -f "$NODEA_GUEST_LOG" ]] && grep -q 'verdict=PASS' "$NODEA_GUEST_LOG" && \
    ...
    sleep 0.5
    validate_npu_logs
    echo "[npu_test] PASS: both nodes completed"
    exit 0
  fi
  if grep -qE 'verdict=FAIL|Kernel panic - not syncing|Call trace:' "$NODEA_GUEST_LOG" "$NODEB_GUEST_LOG" 2>/dev/null; then
    echo "[npu_test] FAIL: guest reported failure" >&2
    exit 1
  fi
  sleep 0.5
done

echo "[npu_test] FAIL: timeout waiting for completion" >&2
exit 1
