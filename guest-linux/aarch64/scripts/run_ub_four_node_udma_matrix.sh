...
REPORT_FILE="${REPORT_FILE:-$OUT_DIR/four_node_udma_matrix.latest.txt}"
TRACE_FILE="${TRACE_FILE:-$OUT_DIR/four_node_udma_matrix.trace.latest.txt}"
RUN_ID_BASE="${RUN_ID:-$(date +%Y-%m-%d_%H-%M-%S)_udma4_${RANDOM}}"
...
run_directed_call() {
  ...
  initiator_slice="$OUT_DIR/four_node_udma_${src}_${dst}_initiator.slice.log"
  responder_slice="$OUT_DIR/four_node_udma_${src}_${dst}_responder.slice.log"
