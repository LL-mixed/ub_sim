...
#
# Runs gsva_lifecycle_test on a real 8-node QEMU setup.
...
for node_suffix in "${NODES[@]}"; do
  ...
  PID_FILES[$node_suffix]="$OUT_DIR/ub_node${node_suffix}.gsva_lc.${RUN_ID}.pid"
done
...
cleanup() {
  local node_suffix
  for node_suffix in "${NODES[@]}"; do
    local pf="${PID_FILES[$node_suffix]}"
    if [[ -f "$pf" ]]; then
      local pid
...
start_node() {
  ...
  env \
    ...
    "$QEMU_BIN" \
      ...
      -append "console=ttyAMA0 rdinit=/bin/run_demo linqu_gsva_lifecycle_test=1 linqu_urma_dp_role=${role} linqu_node_idx=${node_idx} linqu_node_count=8 gsva_test_mode=${GSVA_TEST_MODE} ${APPEND_EXTRA}" \
      >"$qemu_log" 2>&1 &
  echo $! > "$pid_file"
}
...
validate_lifecycle_logs() {
  ...
  for node_suffix in "${NODES[@]}"; do
    qemu_log_args+=("${QEMU_LOGS[$node_suffix]}")
    if grep -Eq 'GSVA_MAP|GSVA_TLB: lookup|GSVA_UNMAP|GSVA_RETIRE' "${QEMU_LOGS[$node_suffix]}" 2>/dev/null; then
      found=true
    fi
  done
  if [[ "$found" == "false" ]]; then
    echo "[gsva_lc8] FAIL: no GSVA lifecycle evidence in QEMU logs" >&2
    return 1
  fi
  if [[ "$GSVA_MODE" == "arm_mmu" ]]; then
    if ! grep -q 'GSVA_TLB: lookup' "${qemu_log_args[@]}"; then
      echo "[gsva_lc8] FAIL: ARM MMU lifecycle lacks GSVA_TLB lookup evidence" >&2
      return 1
    fi
    if grep -q 'GVA_TCG_TRANSLATE' "${qemu_log_args[@]}"; then
      echo "[gsva_lc8] FAIL: ARM MMU lifecycle fell back to GVA_TCG_TRANSLATE" >&2
      return 1
    fi
  fi
}

echo "[gsva_lc8] run_id=$RUN_ID mode=$GSVA_TEST_MODE"
echo "[gsva_lc8] starting 8 nodes..."
...
echo "[gsva_lc8] waiting for FM links..."
if ! wait_all_links_ready "$LINK_WAIT_SECS"; then
  echo "[gsva_lc8] FAIL: FM links not ready" >&2
  exit 1
fi
echo "[gsva_lc8] FM links ready"

echo "[gsva_lc8] waiting for test completion (timeout ${RUN_SECS}s)..."
deadline=$((SECONDS + RUN_SECS))
while (( SECONDS < deadline )); do
  all_pass=true
  for node_suffix in "${NODES[@]}"; do
    if ! [[ -f "${GUEST_LOGS[$node_suffix]}" ]] || ! grep -q 'verdict=PASS' "${GUEST_LOGS[$node_suffix]}"; then
      all_pass=false
      break
    fi
  done
  if [[ "$all_pass" == "true" ]]; then
    ...
    sleep 0.5
    validate_lifecycle_logs
    echo "[gsva_lc8] PASS: all nodes completed"
    exit 0
  fi
  for node_suffix in "${NODES[@]}"; do
    if grep -qE 'verdict=FAIL|Kernel panic - not syncing|Call trace:' "${GUEST_LOGS[$node_suffix]}" 2>/dev/null; then
      echo "[gsva_lc8] FAIL: node${node_suffix} reported failure" >&2
      exit 1
    fi
  done
  sleep 0.5
...
echo "[gsva_lc8] FAIL: timeout waiting for completion" >&2
exit 1
