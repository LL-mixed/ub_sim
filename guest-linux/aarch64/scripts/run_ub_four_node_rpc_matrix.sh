...
REPORT_FILE="${REPORT_FILE:-$OUT_DIR/four_node_rpc_matrix.latest.txt}"
TRACE_FILE="${TRACE_FILE:-$OUT_DIR/four_node_rpc_matrix.trace.latest.txt}"
RUN_ID_BASE="${RUN_ID:-$(date +%Y-%m-%d_%H-%M-%S)_rpc4_${RANDOM}}"
...
PORT_BASE_START="${PORT_BASE_START:-$((52300 + (RANDOM % 300)))}"
...
run_directed_call() {
  ...
  client_slice="$OUT_DIR/four_node_rpc_${src}_${dst}_client.slice.log"
  server_slice="$OUT_DIR/four_node_rpc_${src}_${dst}_server.slice.log"
