...
start_node() {
  ...
  env \
    ...
    "$QEMU_BIN" \
      ...
      -append "console=ttyAMA0 rdinit=/bin/run_app linqu_gva_manager=1 gva_manager_mode=bootstrap linqu_urma_dp_role=${role} gva_manager_node_id=${node_idx} gva_manager_node_count=2 gva_manager_generation=${GVA_MANAGER_GENERATION} gva_manager_aperture_base=${GVA_MANAGER_APERTURE_BASE} gva_manager_aperture_size=${GVA_MANAGER_APERTURE_SIZE} ${segment_append} ${conflict_append} ${APPEND_EXTRA}" \
      >"$qemu_log" 2>&1 &
  echo $! > "$pid_file"
}
