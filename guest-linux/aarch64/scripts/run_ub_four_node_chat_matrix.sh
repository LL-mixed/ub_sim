...
REPORT_FILE="${REPORT_FILE:-$OUT_DIR/four_node_chat_matrix.latest.txt}"
TRACE_FILE="${TRACE_FILE:-$OUT_DIR/four_node_chat_matrix.trace.latest.txt}"
RUN_ID_BASE="${RUN_ID:-$(date +%Y-%m-%d_%H-%M-%S)_chat4_${RANDOM}}"
...
PORT_BASE_START="${PORT_BASE_START:-$((52000 + (RANDOM % 300)))}"
PORT_BASE="$PORT_BASE_START"

NODE_IDS=(nodeA nodeB nodeC nodeD)
NODE_IPS=(10.0.0.1 10.0.0.2 10.0.0.3 10.0.0.4)
DEFAULT_PAIR_LIST=("nodeA nodeB" "nodeA nodeC" "nodeA nodeD" "nodeB nodeC" "nodeB nodeD" "nodeC nodeD")
PAIR_LIST=()
...
run_pair_iteration() {
  ...
  local initiator_slice="$OUT_DIR/four_node_chat_${initiator}_${responder}_initiator.slice.log"
  local responder_slice="$OUT_DIR/four_node_chat_${initiator}_${responder}_responder.slice.log"
  local rc=0
