...
current_initramfs_signature() {
  ...
  write_signature_line "mem_service_qwen3_runtime_range_wait_flow_src" "$MEM_SERVICE_QWEN3_RUNTIME_RANGE_WAIT_FLOW_SRC"
...
"$AARCH64_LINUX_CC" -static -O2 -Wall -Wextra -I"$ROOT_DIR/common" ${=LIBOBMM_CFLAGS} "$OBMM_COH_TEST_SRC" ${=LIBOBMM_SRCS} -o "$OBMM_COH_TEST_BIN"
"$AARCH64_LINUX_CC" -static -O2 -Wall -Wextra -I"$ROOT_DIR/kernel_ub/include/uapi" -I"$ROOT_DIR/common" ${=LIBOBMM_CFLAGS} "$GSVA_QUERY_SRC" ${=LIBOBMM_SRCS} -o "$GSVA_QUERY_BIN"
"$AARCH64_LINUX_CC" -static -O2 -Wall -Wextra -I"$ROOT_DIR/common" ${=LIBOBMM_CFLAGS} "$GSVA_COH_TEST_SRC" ${=LIBOBMM_SRCS} -o "$GSVA_COH_TEST_BIN"
"$AARCH64_LINUX_CC" -static -O2 -Wall -Wextra -I"$ROOT_DIR/common" ${=LIBOBMM_CFLAGS} "$GSVA_LIFECYCLE_TEST_SRC" ${=LIBOBMM_SRCS} -o "$GSVA_LIFECYCLE_TEST_BIN"
...
"$AARCH64_LINUX_CC" -static -O2 -Wall -Wextra -I"$ROOT_DIR/common" ${=LIBOBMM_CFLAGS} "$NPU_GSVA_TEST_SRC" ${=LIBOBMM_SRCS} -o "$NPU_GSVA_TEST_BIN"
"$AARCH64_LINUX_CC" -static -O2 -Wall -Wextra -I"$ROOT_DIR/common" ${=LIBOBMM_CFLAGS} "$SSD_GSVA_TEST_SRC" ${=LIBOBMM_SRCS} -o "$SSD_GSVA_TEST_BIN"
