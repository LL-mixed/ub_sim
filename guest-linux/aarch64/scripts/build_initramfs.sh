...
"$AARCH64_LINUX_CC" -static -O2 -Wall -Wextra -I"$ROOT_DIR" -I"$ROOT_DIR/.." -I"$ROOT_DIR/libs/obmm_queue" -I"$ROOT_DIR/apps/obmm_queue" -I"$ROOT_DIR/libs/lingqu_shmem_pto" -I"$ROOT_DIR/libs/obmm_async" -I"$ROOT_DIR/../../crates/sim-qemu/include" -I"$MEM_SERVICE_ROOT" -I"$MEM_SERVICE_ROOT/libs/obmm_queue" "$LLM_INFER_APP_SRC" "$MEM_SERVICE_SRC" "$MEM_SERVICE_CLUSTER_UTILS_SRC" "$MEM_SERVICE_CLUSTER_PAYLOAD_SRC" "$MEM_SERVICE_CLUSTER_READ_SRC" "$MEM_SERVICE_CLUSTER_RUNTIME_SRC" "$MEM_SERVICE_CLUSTER_QUEUE_SRC" "$MEM_SERVICE_CLUSTER_OBSERVE_SRC" "$MEM_SERVICE_OBMM_OBJECT_FLOW_SRC" "$MEM_SERVICE_CLIENT_SRC" "$MEM_SERVICE_WIRE_CLIENT_SRC" "$MEM_SERVICE_METADATA_SRC" "$MEM_SERVICE_PROVIDER_SRC" "$MEM_SERVICE_KEYS_SRC" "$MEM_SERVICE_OBJECT_REFS_SRC" "$MEM_SERVICE_OBMM_OBJECTS_SRC" "$MEM_SERVICE_UB_SSD_GSVA_BACKEND_SRC" "$MEM_SERVICE_UB_SSD_GSVA_IO_SRC" "$MEM_SERVICE_RECORDS_SRC" "$MEM_SERVICE_PROFILE_SRC" "$MEM_SERVICE_DEEPSEEK_V4_FLASH_SRC" "$MEM_SERVICE_EXPERT_ROUTE_FLOW_SRC" "$MEM_SERVICE_EXPERT_CACHE_SRC" "$MEM_SERVICE_QWEN3_RECORDS_SRC" "$MEM_SERVICE_QWEN3_RUNTIME_SRC" "$MEM_SERVICE_MODEL_DECODE_BARRIER_SRC" "$MEM_SERVICE_MODEL_RANGE_KV_STATE_FLOW_SRC" "$MEM_SERVICE_MODEL_TERMINAL_TOKEN_FLOW_SRC" "$MEM_SERVICE_MODEL_RANGE_WAIT_FLOW_SRC" "$MEM_SERVICE_MODEL_RANGE_PUBLISH_FLOW_SRC" "$MEM_SERVICE_QWEN3_ENGRAM_PUBLISH_FLOW_SRC" "$MEM_SERVICE_QWEN3_ENGRAM_WAIT_FLOW_SRC" "$MEM_SERVICE_QWEN3_SRC" "$LLM_INFER_SRC" "$LINGQU_SHMEM_PTO_DIRECT_LIB_SRC" "$LINGQU_SHMEM_PTO_DIRECT_WIRE_SRC" "$LINGQU_SHMEM_PTO_DIRECT_ENDPOINT_SRC" "$LINGQU_SHMEM_MEM_SERVICE_SRC" "$LINGQU_SHMEM_MEM_SERVICE_OBMM_SRC" "$OBMM_ASYNC_COROUTINE_LIB_SRC" "$OBMM_ASYNC_COROUTINE_ASM_SRC" ${=LIBOBMM_CFLAGS} ${=LIBOBMM_SRCS} ${=LIBOBMM_LDLIBS} -lm -o "$LLM_INFER_APP_BIN"
