...
start_node() {
  ...
  env \
    ...
    "$QEMU_BIN" \
      ...
      -append "console=ttyAMA0 rdinit=/bin/run_demo npu_gsva_test linqu_urma_dp_role=${role} linqu_node_idx=${node_idx} linqu_node_count=4 ${APPEND_EXTRA}" \
      >"$qemu_log" 2>&1 &
  echo $! > "$pid_file"
}
...
validate_npu_gsva_logs() {
  ...
  for node_suffix in "${NODES[@]}"; do
    if ! grep -q 'UB_NPU: created' "${QEMU_LOGS[$node_suffix]}"; then
      echo "[npu_gsva_test_4] FAIL: UB_NPU not created on node${node_suffix}" >&2
      return 1
    fi
  done
  return 0
