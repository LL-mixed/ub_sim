...
#
# Runs gsva_lifecycle_test on a real 4-node QEMU setup.
...
for n in A B C D; do
  ...
  PID_FILES[$n]="$OUT_DIR/ub_node${n}.gsva_lc.${RUN_ID}.pid"
done
...
start_node() {
  ...
  env \
    ...
    "$QEMU_BIN" \
      ...
      -append "console=ttyAMA0 rdinit=/bin/run_demo linqu_gsva_lifecycle_test=1 linqu_urma_dp_role=${role} linqu_node_idx=${node_idx} linqu_node_count=4 gsva_test_mode=${GSVA_TEST_MODE} ${APPEND_EXTRA}" \
      >"$qemu_log" 2>&1 &
  echo $! > "$pid_file"
}
...
validate_lifecycle_logs() {
  ...
  for n in A B C D; do
    ...
    if grep -Eq 'GSVA_MAP|GSVA_TLB: lookup|GSVA_UNMAP|GSVA_RETIRE' "${QEMU_LOGS[$n]}" 2>/dev/null; then
      found=true
    fi
  done
  if [[ "$found" == "false" ]]; then
    echo "[gsva_lc4] FAIL: no GSVA lifecycle evidence in QEMU logs" >&2
    return 1
  fi
  if [[ "$GSVA_MODE" == "arm_mmu" ]]; then
    if ! grep -q 'GSVA_TLB: lookup' "${qemu_log_args[@]}"; then
      echo "[gsva_lc4] FAIL: ARM MMU lifecycle lacks GSVA_TLB lookup evidence" >&2
      return 1
    fi
    if grep -q 'GVA_TCG_TRANSLATE' "${qemu_log_args[@]}"; then
      echo "[gsva_lc4] FAIL: ARM MMU lifecycle fell back to GVA_TCG_TRANSLATE" >&2
      return 1
    fi
  fi
}

echo "[gsva_lc4] run_id=$RUN_ID mode=$GSVA_TEST_MODE"
echo "[gsva_lc4] starting 4 nodes..."
...
echo "[gsva_lc4] waiting for FM links..."
if ! wait_all_links_ready "$LINK_WAIT_SECS"; then
  echo "[gsva_lc4] FAIL: FM links not ready" >&2
  exit 1
fi
echo "[gsva_lc4] FM links ready"

echo "[gsva_lc4] waiting for test completion (timeout ${RUN_SECS}s)..."
...
echo "[gsva_lc4] FAIL: timeout waiting for completion" >&2
exit 1
