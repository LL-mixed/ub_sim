...
# GSVA Lifecycle Test - Two Node
#
# Runs gsva_lifecycle_test on a 2-node QEMU setup.
...
SHARED_DIR="${UB_FM_SHARED_DIR:-/tmp/ub-qemu-links-gsva-lc}"
...
NODEA_PID_FILE="$OUT_DIR/ub_nodeA.gsva_lc.${RUN_ID}.pid"
NODEB_PID_FILE="$OUT_DIR/ub_nodeB.gsva_lc.${RUN_ID}.pid"
...
start_node() {
  ...
  env \
    ...
    "$QEMU_BIN" \
      ...
      -append "console=ttyAMA0 rdinit=/bin/run_demo linqu_gsva_lifecycle_test=1 linqu_urma_dp_role=${role} linqu_node_idx=${node_idx} gsva_test_mode=${GSVA_TEST_MODE} ${APPEND_EXTRA}" \
      >"$qemu_log" 2>&1 &
  echo $! > "$pid_file"
}
...
echo "[gsva_lc] run_id=$RUN_ID mode=$GSVA_TEST_MODE"
echo "[gsva_lc] starting nodeA and nodeB..."
...
echo "[gsva_lc] waiting for FM links..."
if ! wait_for_fm_links_ready "$NODEA_QEMU_LOG" "$NODEB_QEMU_LOG" "$LINK_WAIT_SECS"; then
  echo "[gsva_lc] FAIL: FM links not ready" >&2
  exit 1
fi
echo "[gsva_lc] FM links ready"

echo "[gsva_lc] waiting for test completion (timeout ${RUN_SECS}s)..."
...
while (( SECONDS < deadline )); do
  if [[ -f "$NODEA_GUEST_LOG" ]] && grep -q 'verdict=PASS' "$NODEA_GUEST_LOG" && \
    ...
    sleep 0.5
    validate_lifecycle_logs
    echo "[gsva_lc] PASS: both nodes completed"
    exit 0
  fi
  if grep -qE 'verdict=FAIL|Kernel panic - not syncing|Call trace:' "$NODEA_GUEST_LOG" "$NODEB_GUEST_LOG" 2>/dev/null; then
    echo "[gsva_lc] FAIL: guest reported failure" >&2
    exit 1
  fi
  sleep 0.5
done

echo "[gsva_lc] FAIL: timeout waiting for completion" >&2
exit 1
