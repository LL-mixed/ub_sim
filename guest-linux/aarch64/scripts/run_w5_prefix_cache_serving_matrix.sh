...
Runs a W5 prefix-cache serving matrix:
  1. seed request A with shared prefix and suffix A, GSVA KV writeback, memory reuse disabled
  2. one or more request A replays that must hit GSVA-backed prefix cache
  3. request B with the same shared prefix and divergent suffix B that must miss/recompute
  4. an evicted/empty reuse-store request A that must fail closed when prefix cache is required

The current W5 guest KV stream consumes exact prompt prefix-cache artifacts. Divergent suffix is
therefore a correctness guard: it must not accidentally reuse request A's KV.

Defaults: --steps 8 --same-prefix-runs 2 --suffix-b-token-ids 13.
USAGE
}
...
config_seed_a="$(write_case_config request-a-seed "$prompt_a")"
...
run_case seed-request-a "$config_seed_a" --no-memory-reuse
if (( DRY_RUN )); then
  seed_summary="$OUT_DIR/eight_node_w5_inference_cluster_summary.<seed-request-a-run>.txt"
  seed_run_id="<seed-request-a-run>"
else
...
config_a="$(write_case_config request-a-reuse "$prompt_a" "$OUT_DIR" "$seed_run_id")"
...
while (( reuse_index <= SAME_PREFIX_RUNS )); do
  ...
  if [[ -z "$first_reuse_summary" ]]; then
    ...
    run_case divergent-suffix-request-b "$config_b"
    if (( DRY_RUN )); then
      mismatch_summary="$OUT_DIR/eight_node_w5_inference_cluster_summary.<divergent-suffix-request-b-run>.txt"
      printf '%q ' "$REPORT" --compare-prefix-cache "$seed_summary" "$first_reuse_summary" "$mismatch_summary"
      printf '\n'
    else
      mismatch_summary="$(latest_summary)"
      echo "[w5_prefix_cache_serving_matrix] mismatch_summary=$mismatch_summary"
      "$REPORT" --compare-prefix-cache "$seed_summary" "$first_reuse_summary" "$mismatch_summary"
    fi
  fi
  (( reuse_index += 1 ))
done
