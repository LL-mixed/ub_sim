...
# NPU GSVA Test - Eight Node
#
# Runs npu_gsva_test on a 8-node QEMU setup.
...
start_node() {
  ...
  env \
    ...
    "$QEMU_BIN" \
      ...
      -append "console=ttyAMA0 rdinit=/bin/run_app linqu_npu_gsva_test=1 linqu_urma_dp_role=${role} linqu_node_idx=${node_idx} linqu_node_count=8 linqu_npu_gsva_peer_node_idx=${peer_node_idx} ${APPEND_EXTRA}" \
      >"$qemu_log" 2>&1 &
  echo $! > "$pid_file"
}
...
validate_npu_gsva_logs() {
  ...
  for node_suffix in "${NODES[@]}"; do
    ...
    ub_gsva_trace_require "[npu_gsva_test_8]" "${GUEST_LOGS[$node_suffix]}" \
      "peer_selection=node_idx=${peer_node_idx}" \
      "selected NPU GSVA peer on node${node_suffix}" || rc=1
    ub_gsva_trace_require "[npu_gsva_test_8]" "${GUEST_LOGS[$node_suffix]}" \
      "Testing peer 1/1 node_idx=${peer_node_idx}[[:space:]]" \
      "single NPU GSVA peer execution on node${node_suffix}" || rc=1
    node_idx=$((node_idx + 1))
  done
  return $rc
}
...
echo "[npu_gsva_test_8] starting 8 nodes..."
...
echo "[npu_gsva_test_8] waiting for FM links..."
if ! wait_all_links_ready "$LINK_WAIT_SECS"; then
  echo "[npu_gsva_test_8] FAIL: FM links not ready" >&2
  exit 1
fi
echo "[npu_gsva_test_8] FM links ready"

echo "[npu_gsva_test_8] waiting for test completion (timeout ${RUN_SECS}s)..."
...
while (( SECONDS < deadline )); do
  ...
  if [[ "$all_pass" == "true" ]]; then
    ...
    sleep 0.5
    validate_npu_gsva_logs
    echo "[npu_gsva_test_8] PASS: all nodes completed"
    exit 0
  fi
  for node_suffix in "${NODES[@]}"; do
    local guest_log="${GUEST_LOGS[$node_suffix]}"
    if grep -qE 'verdict=FAIL|Kernel panic - not syncing|Call trace:' "$guest_log" 2>/dev/null; then
      echo "[npu_gsva_test_8] FAIL: node${node_suffix} reported failure" >&2
      exit 1
    fi
  done
  sleep 0.5
...
echo "[npu_gsva_test_8] FAIL: timeout waiting for completion" >&2
exit 1
