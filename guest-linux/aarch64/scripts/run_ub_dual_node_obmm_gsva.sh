...
OBMM_GSVA_MODE="${OBMM_GSVA_MODE:-${GSVA_DEMO_MODE:-identity}}"
OBMM_GSVA_BASE="${OBMM_GSVA_BASE:-${GSVA_DEMO_BASE:-0x700000000000}}"
OBMM_GSVA_SIZE="${OBMM_GSVA_SIZE:-${GSVA_DEMO_SIZE:-0x400000}}"
OBMM_GSVA_NODE_COUNT="${OBMM_GSVA_NODE_COUNT:-${GSVA_DEMO_NODE_COUNT:-2}}"
...
start_node() {
  ...
  env \
    ...
    "$QEMU_BIN" \
      ...
      -append "console=ttyAMA0 rdinit=/bin/run_demo obmm_gsva linqu_urma_dp_role=${role} linqu_node_idx=${node_idx} obmm_gsva_mode=${OBMM_GSVA_MODE} obmm_gsva_base=${OBMM_GSVA_BASE} obmm_gsva_size=${OBMM_GSVA_SIZE} obmm_gsva_node_count=${OBMM_GSVA_NODE_COUNT} ${APPEND_EXTRA}" \
      >"$qemu_log" 2>&1 &
  echo $! > "$pid_file"
}
