...
class PaperEngramHashHeaderTest(unittest.TestCase):
    ...
    def test_vendor_cxx_header_matches_rust_hash_vectors(self):
        cxx = shutil.which("c++")
        if cxx is None:
            self.skipTest("c++ is not available")
        repo = Path(__file__).resolve().parents[3]
        header_dir = repo / "vendor" / "pto-isa" / "kernels" / "manual" / "a5" / "engram_simt"
        source = textwrap.dedent(
            r"""
            #include <inttypes.h>
            #include <stdint.h>
            ...
            int main()
            {
                ...
                uint64_t three[] = {1, 2, 3};
                uint64_t other_three[] = {10, 11, 12};
                printf("algorithm=%s\n", ENGRAM_HASH_ALGORITHM_V1);
                printf("exact_1_2=0x%016" PRIx64 "\n",
                       engram_ngram_exact_key_v1(two, 2));
                printf("exact_2_1=0x%016" PRIx64 "\n",
                       engram_ngram_exact_key_v1(two_reversed, 2));
                printf("exact_1_2_3=0x%016" PRIx64 "\n",
                       engram_ngram_exact_key_v1(three, 3));
                printf("exact_10_11_12=0x%016" PRIx64 "\n",
                       engram_ngram_exact_key_v1(other_three, 3));
                printf("row_2_head0=%" PRIu64 "\n",
                       engram_row_hash_v1(2, 0, two, 2, 1024,
                                          UINT64_C(0x12345678)));
                printf("row_2_head1=%" PRIu64 "\n",
                       engram_row_hash_v1(2, 1, two, 2, 1024,
                                          UINT64_C(0x12345678)));
                printf("row_3_head0=%" PRIu64 "\n",
                       engram_row_hash_v1(3, 0, three, 3, 1024,
                                          UINT64_C(0x12345678)));
                return 0;
            }
            """
        ...
        with tempfile.TemporaryDirectory() as tmp:
            src = Path(tmp) / "engram_common_probe.cc"
            bin_path = Path(tmp) / "engram_common_probe"
            src.write_text(source, encoding="utf-8")
            subprocess.run(
                [
                    cxx,
