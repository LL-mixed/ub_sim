...
def test_mem_service_has_component_and_cli_entrypoints():
    ...
    assert "^package_manifest_checksum=0x221916e1$$" in app_makefile
    ...
    assert "MEM_SERVICE_UPGRADE_ROLLBACK_POLICY_EXPECTED_CHECKSUM 0xf7943816U" in app_source
    ...
    assert "MEM_SERVICE_PACKAGE_MANIFEST_EXPECTED_LEN 9122U" in app_source
    assert "MEM_SERVICE_PACKAGE_MANIFEST_EXPECTED_CHECKSUM 0x221916e1U" in app_source
    ...
    assert "package_manifest_checksum=0x221916e1" in release_manifest
    ...
    assert "record_retention_policy=manual-or-global-latest-or-kind-latest" in release_manifest
    ...
    assert "upgrade_rollback_policy_checksum=0xf7943816" in release_manifest
    assert "package_manifest_len=9122" in release_manifest
    assert "package_manifest_checksum=0x221916e1" in release_manifest
    ...
    assert (
        ...
        "upgrade-rollback-policy.txt checksum=0xf7943816"
    ) in package_manifest
    ...
    assert "field=record_retention type=enum values=manual,latest:<records>,kind:<record-kind>:latest:<records>" in config_schema
    ...
    assert "record_retention_policy=manual-or-global-latest-or-kind-latest" in package_manifest
