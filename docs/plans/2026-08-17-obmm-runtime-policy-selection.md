# OBMM remote-load sync/P2A/P2B work-conserving 运行时选择表
...
> 下的 workload makespan 重新聚合；Arm64 native P2B path-tax microbenchmark 已完成；
...
sync、P2A 和 P2B 应在同一数据面共存，并在 mapping/session 的 quiescent point 选择。
P2A/P2B 会增加单次 remote load 的 suspend、schedule 和 resume 开销；它们的目标是让
...
正式运行。70 个 endpoint 中的 measured-fastest 分布为 sync 29、P2A 7、P2B 34；应用
发布阈值后，transparent policy 为 sync 32、P2B 38，explicit policy 为 sync 32、
P2A 7、P2B 31。三个 measured-fastest=P2B 的 endpoint 因收益或置信区间不足回退 sync。
...
| 100 µs | 2/4/8/32 | 0/10/100/1000 µs | P2B | P2B | P2B workload gain 为 11.1%--41.4%，paired CI 全部通过 |
| 1000 µs | 2 | 0/10/100/1000 µs | P2B | P2A | P2A 的 workload 总完成时间最短；W=1000 µs 选择 demand，其余选择 lookahead=16 |
| 1000 µs | 4 | 0/10/100 µs | P2B | P2A | P2A makespan 最短 |
| 1000 µs | 4 | 1000 µs | P2B | P2B | P2B makespan 最短 |
| 1000 µs | 8/32 | 0/10/100/1000 µs | P2B | P2B | P2B makespan 最短 |
| 未测 bucket | 任意 | 任意 | sync | sync | 缺少完整 paired evidence，fail closed |

![QEMU PoC 的 sync、P2A、P2B work-conserving 运行时选择流程](2026-08-17-obmm-runtime-policy-selection.svg)
...
| Remote latency | Formal endpoints | Measured fastest：sync / P2A / P2B | Published explicit：sync / P2A / P2B | Published transparent：sync / P2B |
|---:|---:|---:|---:|---:|
...
三个 endpoint 的 measured-fastest 为 P2B；前两个的 paired median gain 只有 1.1% 和
1.2%，第三个的 median gain 为 1.0% 且 CI95 low 为 -0.2%，因此全部 fail closed 到
sync。L=50 µs 已进入 C/W 相关的混合区：`W=300` 下 C=`3/5/6/12/16/24`，以及
`C=32/W=1000` 的 endpoint 发布 P2B，其余 18 个 endpoint 发布 sync。L=75 µs 的 17 个
正式 endpoint 全部发布 P2B。

L≥250 µs 的低 C endpoint 开始出现 P2A/P2B 分化。transparent surface 无法选择 P2A，
所以这 7 个 P2A measured-fastest endpoint 在普通 `LDR` 上发布 P2B；explicit surface
按精确 bucket 选择 P2A。任何没有出现在 `summary/policy.json` 的 bucket 继续选择 sync。
...
`0.42--0.43 µs/load`。该 envelope 没有覆盖 SCC MMIO、真实 upcall entry、ack、barrier
和 resume，因此当前不能直接把 `0.43 µs` 当作完整硬件路径税。

暂定 policy 使用以下两个数：

- `O_envelope=0.43 µs`：本轮实测 lower bound；
- `O_budget=2 µs`：完整 native SCC 路径的临时工程预算，等待真实硬件 benchmark 替换。

下表假设 correctness、failure、drain 和资源包络 gate 全部通过。`R` 表示除当前 blocked
coroutine 以外的 runnable coroutine 数量。表中的“P2B 优先”还要求近期 telemetry
...
| `R>0` | `L≤2 µs` | 任意 | sync 默认；允许受控 P2B probe | sync 默认；允许受控三路径 probe | 完整 SCC 路径尚未实测，等待窗口相对 `O_budget` 太小 |
| `R>0` | `2<L<10 µs` | 任意 | P2B online probe | P2A/P2B online probe | native envelope 已有收益空间，静态发布仍缺少端到端硬件证据 |
| `R>0` | `10≤L<250 µs` | 任意 | P2B 优先 | P2B 优先 | `L/O_budget≥5`；QEMU 在 L=30 µs 的 sync 结果受到 70--84 µs 模拟路径税主导 |
| `R>0` | `250≤L<1000 µs` | `C≥4` | P2B 优先 | P2B 优先 | QEMU formal endpoint 在 L=500/C=4/W=0/10 发布 P2B；native 仍需 paired 复测 |
| `R>0` | `250≤L<1000 µs` | `C=2` | P2B 优先 | P2A/P2B online select | QEMU formal endpoint 在 L=250/C=2/W=0/10 与 L=500/C=2/W=100 选择 P2A，在 L=250/C=2/W=100 与 L=500/C=2/W=1000 选择 P2B |
| `R>0` | `L≥1000 µs` | `C≥8`，或 `C=4/W=1000 µs` | P2B | P2B | 7-seed QEMU coarse winner 为 P2B，native 更低路径税继续有利于 P2B |
| `R>0` | `L≥1000 µs` | `C=2`，或 `C=4/W≤100 µs` | P2B | P2A/P2B online select | QEMU coarse 的 P2A schedule-ahead 有优势；native P2B 校准可能移动该边界 |

P2A 的目标区域是显式接口、低 coroutine 并行度、长 remote latency，以及 lookahead 能
显著增加 backend pending depth 的负载。P2B 的目标区域是普通 `LDR` 透明访问、存在
runnable coroutine，且等待窗口足以覆盖 upcall/switch/resume。sync 覆盖低延迟、空
ready queue、未测 bucket 和异常回退。

这张 native 表属于**校准后的部署 prior**。正式 native policy 仍需真实 SCC/MMIO 的
端到端 path-tax、P2A native submit/CQ 成本和 paired makespan 数据。运行时应记录实际
...
- 相同 seed 的 sync/P2A/P2B paired delta；
- 7-seed bootstrap CI；
...
### 3.1 P2B 路径税与 break-even

P2B 只有在成功隐藏的 remote-wait 时间超过增量路径税后，才能缩短 workload
makespan：

```text
P2B net gain = H(L, C, W) - O_p2b(C, W, implementation)

P2B wins when H(L, C, W) > O_p2b
```

- `H(L,C,W)` 是 remote load 等待期间实际执行其他 coroutine 工作后隐藏的时间；
- `O_p2b` 包含 pending/completion upcall、上下文保存与恢复、READY 选择和 resume；
- `H` 受 coroutine 数量、READY useful work 和 backend 并行度限制，通常随 `L` 增长；
- `O_p2b` 越高，sync/P2B break-even 所需的 `L` 越大。

当前 QEMU/TCG PoC 的 `O_p2b` 较高。L=0、W=0/10 µs 的 7-seed workload 中，P2B
...
L=10 µs 的 65,536 次 load 只有 0.655 s modeled wait。此时 P2B 路径税没有被等待
窗口覆盖：

| L | C | W | sync makespan | P2B makespan | P2B 相对 sync |
|---:|---:|---:|---:|---:|---:|
...
P2B 只增加 0.414 s。P2B 已经覆盖大部分新增 remote wait，并在 L=100 µs 跨过
...
2. [`obmm_scc_aarch64.S`](../../guest-linux/aarch64/libs/obmm_scc/obmm_scc_aarch64.S)
   保存 GPR、SIMD、FP 和 TLS，共 832 bytes；
3. [`obmm_scc.c`](../../guest-linux/aarch64/libs/obmm_scc/obmm_scc.c) 中的 EL0 scheduler
...
`ioctl-envelope` 使用 pipe `FIONREAD` 提供两次真实 system-call crossing。它没有执行 SCC
...
因此 QEMU 的 10--100 µs sync/P2B crossing 只适用于当前模拟器。native 部署 prior
使用 `O_budget=2 µs` 保留 MMIO/upcall 余量，并将静态 P2B 启用点暂定为 `L≥10 µs`；
`2<L<10 µs` 使用 online probe，`L≤2 µs` 默认 sync。真实 SCC benchmark 完成后必须用
实测分位数替换 `O_budget`。

P2B campaign 保持 `extra_vcpus=0`。`el0_scheduler_ns` 是从 upcall dispatch 到 resume
的 elapsed time，可能包含 `GET_EVENT` 等待；它不能当作额外 CPU 资源，也不能单独用来
推导纯 context-switch 成本。

### 3.2 空 ready queue 与 no-switch fast path

`O_p2b<L` 只说明等待窗口具有容纳路径税的容量。没有 runnable coroutine 时，隐藏的
有效工作 `H=0`，completion/resume 仍可能暴露在关键路径上。把 pending 和 completion
侧成本分别写成 `P` 和 `R_resume`：

```text
T_sync              = L
T_p2b(no runnable)  ≈ max(L, P) + R_resume
```

pending 侧成本可以与已经发出的 remote request 重叠。结果 ready 之后的 resume 成本
通常无法继续藏在同一个等待窗口中。因此，即使 `P+R_resume<L`，空 ready queue 的 P2B
...
5. core/SCC 若支持 runnable hint，可在 `runnable_count=0` 时抑制 pending upcall；外部
   completion 使其他 coroutine 变为 READY 后再按需触发。

对应的收益条件为：

```text
capacity condition: L > O_p2b
...
| P2A | 21.022 s | 73.8% | 6.207 ms |
| P2B | 41.640 s | 48.0% | 1.413 ms |

透明普通 `LDR` 只能在 sync/P2B 中选择，因此该 bucket 选择 P2B；显式接口允许 P2A，
因此选择 makespan 更短的 P2A。两条 P2 路径的单次 load 都承担调度代价，同时在同一个
...
| P2A | 7 | L=1000 µs 下 C=2 全部，以及 C=4、W=0/10/100；其中 6 个 bucket 选择 lookahead=16，C=2/W=1000 选择 demand |

L=100 µs 的 16 个 bucket 中，P2B paired median makespan gain 为
`11.1%--41.4%`，95% CI 下界为 `10.9%--40.2%`。L=1000 µs 的 16 个 bucket 中，
P2B gain 为 `33.5%--85.2%`；P2A gain 为 `43.1%--80.7%`。运行时仍需按完整 bucket
...
| Transparent policy | 32 | 0 | 38 | 普通 `LDR` 只能在 sync/P2B 中选择 |
| Explicit policy | 32 | 7 | 31 | submit/await 可在三条路径中选择 |

三个 P2B measured-fastest endpoint 没有达到“median gain ≥10% 且 paired CI95 low ≥5%”
的发布要求，策略回退 sync。这个差异说明 measured winner 只能用于定位 crossing；线上
policy 必须使用带置信区间的 eligibility 结果。

L=30 µs 的 formal endpoint 全部发布 sync，L=75 µs 的 formal endpoint 全部发布 P2B。
L=50 µs 形成清晰的 C/W 混合层：25 个 endpoint 中 7 个发布 P2B、18 个发布 sync。
L≥250 µs 的低 C 区域出现 P2A/P2B 分化，证明 P2A 的 schedule-ahead 优势同时依赖
latency、coroutine 数量、useful compute 和 lookahead；沿 L 单轴外推会给出错误选择。

## 5. 趋势与三维离散选择空间

现有 coarse 数据呈现四个区域：低延迟由 sync 占优；100 µs 层由 P2B 占优；
1000 µs 层需要继续看 coroutine 数量和 useful compute，低 C/W 区域由 P2A 的
schedule-ahead 填补 P2B 的 no-ready 空洞，高 C 或高 W 区域重新由 P2B 占优。

![sync、P2A、P2B 随 L/C/W 变化的分支趋势](2026-08-18-obmm-runtime-policy-trend.svg)

下图把 QEMU explicit policy 的 80 个正式已测 bucket 投影到 `L × C × W` 三维离散
空间。每个圆点对应一个真实 7-seed bucket；颜色只表示该点的 workload 总完成时间
最短路径。相邻点之间没有插值。普通 `LDR` 的 transparent surface 不包含 P2A：
L=0/1/10 µs 的 48 个点选择 sync，L=100/1000 µs 的 32 个点选择 P2B。
...
`runnable_count>0` 时的部署 prior：绿色为 sync，蓝色为 P2B，黄蓝分割点要求 P2A/P2B
online select，绿蓝分割点要求 sync/P2B probe；图顶的 override 表示空 ready queue
...
细粒度 formal endpoint 把当前 QEMU PoC 的 sync/P2B crossing 收敛到 30--75 µs 之间，
L=50 µs 是明显的 C/W 混合层。native 图把保守静态 P2B 启用点移到 10 µs，2--10 µs
保留 probe；这一移动来自 0.42--0.43 µs envelope 和 2 µs 工程预算。QEMU formal 数据
不能替代 native SCC 的端到端 paired 测量，因此 native 图继续标为部署 prior。
...
5. `runnable_count=0` 时触发 sync fast-path override，跳过普通 P2B save/switch；
6. native profile 在真实 SCC 数据到位前使用 `O_budget=2 µs`，并持续记录 probe 结果；
...
2. **Quiescent switching**：在 mapping/session quiescent point 切换 sync/P2A/P2B，
   验证旧路径 pending/completion 全部 drain，禁止逐 load 抖动切换；
3. **Online telemetry**：采集 `L`、ready depth、no-ready 比例、P2A pending depth、
   P2B exposed path tax 和实际 makespan，支持受控 probe 与策略更新；
4. **Mechanism attribution**：补充 pending/completion upcall、context save/schedule/
   restore、resume 和 P2A submit/CQ 的分段时间戳，解释 L=50 µs 混合层的因果来源；
5. **Native SCC formal policy**：在真实 MMIO/upcall/resume 路径上执行 paired 7-seed
