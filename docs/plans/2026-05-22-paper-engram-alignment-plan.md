...
- Trained/imported quality claims also reject fixture-provenance table artifacts, including both `fixture://...` refs and local `fixture/...` bundle paths, so `seed-paper-engram-fixture` output can still validate runtime wiring but cannot be reused as evidence for model-quality improvement.
