...
- `Finetune` quality claims now accept either `EngramLora` or `FullFinetune` recipe evidence, while `Posttrain` remains restricted to Engram-only continued pretrain and `Imported` remains restricted to external-import provenance.
...
- Trained/imported quality claims now require runtime acceptance evidence in the eval report: all four Phase 6 loss variants (`base`, `base+decode_policy`, `base+paper_engram`, `base+paper_engram+decode_policy`) must be present, and the same report must carry CPU/backend output match, non-zero-table hidden/output checksum deltas, row-prefetch locality counters, and bounded backend latency.
- Trained/imported quality claims also reject fixture-provenance table artifacts and training/eval evidence, including both `fixture://...` refs and local `fixture/...` bundle paths, so `seed-paper-engram-fixture` output can still validate runtime wiring but cannot be reused as evidence for model-quality improvement.
...
- The vendor `engram-simt` host harness now accepts an optional per-case `table.bin`, and `sim-models::engram_simt_adapter` can materialize runtime fused-SIMT case data (`table.bin`, `indices.bin`, `hidden.bin`, `gate_weight.bin`, `golden.bin`) before launching the selected vendor case. When W5/UAPI sees the exported `SIM_ENGRAM_SIMT_*` artifact environment, fused-simt context execution now runs that materialized vendor case and reports `fused-simt-vendor-*`; without the artifact environment it remains on the explicit ABI-reference path. W5 summary/report/health tooling now surfaces fused-SIMT context latency and byte counters, and `--require-context fused_simt_vendor_context` makes vendor-path evidence a machine-checked acceptance gate for real cluster runs.
...
artifact environment is present. W5 report/health tooling now exposes and can
require `fused_simt_vendor_context` evidence so a real cluster run cannot be
mistaken for the ABI-reference fallback. Remaining work is to run the real W5
cluster path with rebuilt vendor artifacts and collect backend
latency/equivalence evidence.
