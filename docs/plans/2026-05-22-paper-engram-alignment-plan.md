...
- `sim-cli lingqu-memory import-paper-engram-module` can import a complete paper Engram manifest bundle in dependency order (`projection`, `hash_config`, table shards, gates, optional recipe/eval, module), persist all registries, and resolve runtime artifacts as the import validation gate. `validate-paper-engram-module` exposes the same runtime validation directly and can additionally require the trained/imported quality gate with `--require-quality`.
...
order and validates them by resolving runtime artifacts. `validate-paper-
engram-module` validates an already registered module's runtime operand bundle
and can require the quality evidence gate. `resolve-paper-engram-
