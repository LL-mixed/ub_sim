...
minimal --store snapshot+journal exists and carries completed idempotency records plus bounded audit records; storage_root layout manifest, derived store recovery, and sealed-local-block-v1 small payload write/verify exist, but no production durable catalog/chunked sealed block backend or journal truncation/atomicity policy
