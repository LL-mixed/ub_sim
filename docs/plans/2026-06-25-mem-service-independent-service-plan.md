...
6. 定义 idempotency：
   ...
   - 后续重点转为真实外部系统接入后的 SLA、capacity、TTL/tenant 分层 retention 策略、真实 at-rest/data-plane encryption/key management 和多版本迁移策略。
...
remaining blockers are external Linux ops evidence, cross-host production remote transport evidence, real serving/pretraining SLA/capacity evidence, and longer-term TTL/tenant retention policy, real encryption/key management, and migration policy
```
