...
- QEMU、kernel 和 guest 的 P2B scheduler-core 路径解决的是“远端普通 `LDR`
...
> `mem_service` daemon 和 typed SDK 解析 ObjectRef，经 OBMM/P2B EL0 coroutine
...
| 项目 | 当前值 | 审计判断 |
| --- | --- | --- |
| `ub_sim` HEAD | `b20af7c76d5af300c8b307667e8d06b8fb71b77a` | 包含 ABI v2 P3 已提交证据 |
| 与 `origin/master` 关系 | ahead 6；`origin/master=67c3ce5` | 远端主分支仍落后于当前完整基线 |
| `mem_service` gitlink/lock/HEAD | `91c20cd34fe6ad68405d0d17b3ad5481f889163c` | 三者一致，锁定关系成立 |
| QEMU 子模块 | `b3bb87b4970c09a5c8226e81905e644c03c55efc` | 包含 OBMM split-phase 和 EL0 upcall 模型 |
| guest kernel 子模块 | `3bc9ebd1126f8b1ab3769099f801eb9bba983220` | 包含 OBMM async 和 scheduler UAPI |
...
在本报告开始修改前，工作区已经有 9 个未提交的 P3 相关文件，覆盖 evaluator
安全 resume、immutable raw/attempt evidence、guest SCC progress/deadline 和 trace
统计等修正。这些改动通过了本报告第 10 节列出的 focused tests，但**尚未提交，
也没有完成 4,942-case 运行**，因此不能并入“已发布基线”或“P3 已完成”的结论。

## 4. 两者的实际分工与连接点

![ub_sim 与 Lingqu DataSystem 的分工、provider 分流及 P2B EL0 调度路径](2026-08-14-ub-sim-lingqu-datasystem-relationship.svg)
...
| P2B scheduler core | QEMU + kernel UAPI + `obmm_scc` + `obmm_async_coroutine` | 已形成独立 2-node ABI v2 验证路径 |
| W5 与 P2B 合流 | 没有当前 revision 的无旁路模型验收 bundle | **尚未证明** |
...
| `6ef9c13`，2026-08-13 | P2A/P2B remote-load coroutine | 增加 slow remote load 的 EL0 调度机制 |
...
- P2A explicit submit/await 与 P2B ordinary-load/direct-EL0-upcall 均已实现；P2B 的
  context save/switch/resume 由 guest EL0 scheduler 完成，QEMU 负责建模事件投递、
  pending load 和精确完成。详细设计见
  [P2B scheduler-core design](plans/p2b-scheduler-core-detailed-design.md)。
...
- Qwen3-14B 8-node、16-step W5 seed/reuse/Engram 有历史报告，见
  ...
  revision，覆盖范围没有包含当前独立 `mem_service` + P2B 联合验收。
...
1. **缺少统一入口。** 当前独立 daemon bootstrap、guest publish/restart smoke、
   Rust `lingqu-memory` service、W5 runner 和 P2B evaluator 是多个入口，没有一个
...
3. **P2B 尚未与真实模型消费点合流。** 当前 P2B test program 能证明普通远端
...
- P3 全部 4,942-case sensitivity/break-even 矩阵；它回答性能区间，不决定功能链路
  是否存在。
...
| G3 remote map/load | nodeB 由 OBMM provider 获取 mapping descriptor，并以普通 `LDR` 消费 | OBMM/P2B test path 已通过 |
| G4 EL0 scheduling | 至少两个 coroutine；日志证明 pending upcall、EL0 save/switch、complete upcall、resume | P2B ABI v2 test path 已通过 |
| G5 model correctness | Qwen3 产生固定 token/checksum，且与同步 reference 一致 | 历史模型验证已有，P2B 联合 run 未验 |
...
- 至少一次跨节点 P2B load 和一次 Block/DFS recovery；
...
- **组件就绪度约 65%～75%**：服务 core、provider、QEMU/kernel、P2B runtime、
...
| 困难 | 为什么难 | 处理原则 |
| --- | --- | --- |
| 控制面与数据面身份一致 | daemon ObjectRef、OBMM mapping key、P2B pending load 和 durable ref 必须指向同一对象版本 | 统一 `run_id/object_id/version/generation`，跨层日志可 join |
...
| M1 2-node golden path | 独立 daemon + ObjectRef + OBMM/P2B + Qwen3 正确结果 | 5～7 天 |
...
| M4 定向性能 | sync/P2A/P2B 的代表点与服务开销分解 | 3～5 天 |
...
| P2B `49/49`、4/8-node `14/14` | 已提交报告和 artifact hash 支持 | 保留为 C 类历史运行证据，不写成当前工作区重跑 |
| Qwen3 W5 8-node/16-step 已通过 | 历史报告支持 | 保留为 C 类证据；不等价于当前独立服务 + P2B PoC |
...
python3 -m unittest \
  ...
  guest-linux.aarch64.tests.test_obmm_scc_contract
# 19 tests, OK
...
把独立服务、对象语义、OBMM/P2B、真实 workload、Block/DFS 恢复和证据聚合串成
一条链；新增并列功能应排在这条纵向链路之后。

一旦 2-node golden path 达成，8-node 是规模化验证；P3 是性能边界验证；DeepSeek
是更重 workload 的兼容性验证。三者不应反过来阻塞最小纵向 PoC。
