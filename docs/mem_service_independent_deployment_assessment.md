...
Rust 侧已有 `sim-memory` / durable store / prefix cache / execution artifact 相关模型；guest `mem_service` 当前有最小 snapshot+journal recovery、bounded retained audit ring 和 `storage_root` durable catalog layout manifest，但 payload block backend 仍是 `layout-only`，还没有接入产品级 durable catalog、sealed payload block backend、schema migration 和 journal truncation/atomicity policy。
...
2. 引入 sealed payload block backend。
