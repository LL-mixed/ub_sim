...
- **已闭环（2026-06-27，4.2b / 2026-06-29 增量）**：chunked sealed block backend（`sealed-chunked-block-v1`，payload_kind=65）：大 payload 按 1024B 分块写入 `blocks/<checksum>.chunked/` + manifest（chunk_count/chunk_size/total_len/total_checksum），validate 按序重组并重算 FNV-1a 64-bit 校验、任何不一致即 fail-closed quarantine 整目录。新增 `chunked-block-fixtures` 用真实文件 I/O 证明写/校验/破坏后隔离。`payload_block_backend` 契约扩展为 `sealed-local-block-v1,sealed-chunked-block-v1,transport-loopback-block-v1`；其中 `transport-loopback-block-v1`（payload_kind=66）提供 transport backend 骨架，写入 `remote-blocks/<checksum>.transport/` + manifest，`transport-block-fixtures` 证明写/校验/破坏后 fail-closed quarantine。
...
- remote（远端 transport）block backend 已从纯 policy 文本推进到 loopback transport 数据面：`remote_payload_block_backend=transport-loopback-block-v1`、`remote_payload_block_data_gate=transport-block-fixtures` 与 `remote_payload_block_backend_gate=remote-block-backend-policy-fixtures` 进入 release manifest。跨机 network transport 仍显式 `remote_payload_network_transport=not-certified`，避免把 loopback transport 误读为多机远端 backend 已可用。
