...
12. ✅ Linux CI wrapper 已补齐（2026-06-29，4.4k）：新增 `scripts/run_mem_service_linux_ops_ci.sh --rollback-rpm <previous-rpm> [--out-dir <dir>]`，作为真实 Linux CI 推荐入口，封装 `PACKAGE_OUT_DIR` 与 `OPS_CERTIFICATION_ROLLBACK_RPM`，运行 `linux-ops-deployment-smoke`，并明确输出 `ops-certification-linux-ci.evidence` 与 `ops-certification-upgrade-rollback.marker` artifact 路径；`--dry-run` 可在非 Linux 环境验证命令展开。
