...
- 发布/安装能力已打通：release manifest、package manifest、install-smoke、package-tarball-smoke、package-deb-smoke、package-rpm-smoke（rpm 需要 Linux rpm toolchain）。安装布局已包含 systemd unit 默认引用的 `/etc/lingqu/mem_service/mem_service.conf` runtime config；该 runtime config 来自独立的 `configs/mem_service.runtime.conf`，不再复用 `/tmp` 示例配置，并把 enableable unit 安装到 `usr/lib/systemd/system/`；tar/deb/rpm smoke 会验证配置与 unit 随包落盘。
...
- systemd 默认配置安装布局已补齐：`install` 会把 `configs/mem_service.runtime.conf` 作为 runtime config 安装到 `etc/lingqu/mem_service/mem_service.conf`，同时把 runtime config 原件放入 `share/lingqu/mem_service/config/` 并保留 `/tmp` 路径的 example config 作为开发样例；release/package manifest 记录 `runtime_config` 与 `runtime_config_source=share/lingqu/mem_service/config/mem_service.runtime.conf`，tar/deb/rpm package smoke 均验证该文件存在。该项解决了 unit 文件指向 `/etc/.../mem_service.conf` 但安装包只带 share/example config 的部署缺口。
- 标准 systemd unit 安装布局已补齐：`install` 会把 `linqu_mem_service.service` 与 `linqu_mem_service.host.service` 安装到 `usr/lib/systemd/system/`，同时保留 `share/lingqu/mem_service/deploy/` 下的 contract manifest 副本；unit 声明 `RuntimeDirectory=lingqu` 与 `StateDirectory=lingqu/mem_service`，让 Linux systemd 负责 `/run/lingqu` 和 `/var/lib/lingqu/mem_service` 默认目录。release/package manifest 记录 `systemd_unit` 与 `host_systemd_unit`，tar/deb/rpm package smoke 均验证 unit 随包落盘。真实 `systemctl enable/start/status` 仍属于 Linux CI 外部认证范围。
...
8. ✅ 默认 runtime config 安装闭环已补齐（2026-06-29，4.4g）：`install`、tar/deb/rpm package payload、release/package manifest、install-smoke 和 package smoke 均覆盖 `etc/lingqu/mem_service/mem_service.conf`，与 systemd unit 的 `--config /etc/lingqu/mem_service/mem_service.conf` 默认路径对齐；正式 runtime config 来自 `configs/mem_service.runtime.conf`，默认使用 `/run/lingqu/mem_service.sock` 与 `/var/lib/lingqu/mem_service`，不再复用 `/tmp` 示例配置。
9. ✅ 标准 systemd unit 安装闭环已补齐（2026-06-29，4.4h）：`install`、tar/deb/rpm package payload、release/package manifest、install-smoke 和 package smoke 均覆盖 `lib/systemd/system/linqu_mem_service.service` 与 `lib/systemd/system/linqu_mem_service.host.service`，使包安装后具备直接启用的 systemd unit 文件；unit 声明 `RuntimeDirectory=lingqu` 和 `StateDirectory=lingqu/mem_service`，让 Linux systemd 管理默认 socket/state 目录。真实启停与状态验证仍由 Linux CI 的 `linux-ops-certification-smoke` 认证。
