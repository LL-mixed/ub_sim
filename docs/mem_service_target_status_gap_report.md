...
- remote（远端 transport）block backend 已从纯 policy 文本推进到 loopback + TCP loopback 数据面：`remote_payload_block_backend=transport-loopback-block-v1,transport-tcp-block-v1`、`remote_payload_block_data_gate=transport-block-fixtures`、`remote_payload_network_transport=tcp-loopback-certified`、`remote_payload_network_transport_gate=network-transport-block-fixtures`、`remote_payload_network_transport_make_gate=network-transport-block-smoke` 与 `remote_payload_block_backend_gate=remote-block-backend-policy-fixtures` 进入 release manifest。跨主机生产网络 transport 仍需真实部署验证，当前新增 `remote_payload_production_network_transport=not-certified`、`remote_payload_production_transport_evidence_schema=remote-transport-evidence-v1`、`remote_payload_production_transport_evidence_gate=remote-transport-evidence-fixtures`、`remote_payload_production_transport_generate=remote-transport-generate-evidence`、`remote_payload_production_transport_verify=remote-transport-verify --evidence-file` 和 `remote_payload_production_transport_ci=scripts/run_mem_service_remote_transport_ci.sh`。generator 会真实拉取 producer TCP payload、验证 sealed copy、破坏本地 block 证明 fail-closed，并拒绝 loopback source；CI wrapper 封装 build/generate/verify 三步；verifier 用可复验 evidence 边界避免把 loopback TCP 误读为多机网络已认证。
