...
13. ✅ Linux CI evidence 独立复验入口已补齐（2026-06-29，4.4l）：新增 `scripts/verify_mem_service_linux_ops_evidence.sh --evidence-file <path> [--app-dir <dir>]`，用于 CI 后或跨机器拿到 artifact 后重新构建/定位 `linqu_mem_service_host` 并调用 `ops-certification-verify --evidence-file <path>`。它不接受人工状态覆盖，evidence 缺失或 verifier 拒绝都会 fail-closed。
