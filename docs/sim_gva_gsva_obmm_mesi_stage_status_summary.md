...
30. Active UB Link GSVA remote retire/ACK 已验证：nodeA shared retire 通过 UB Link 发送 `sub=15 op=9` RETIRE 到 nodeB；nodeB RX dispatch 进入 `rx RETIRE` 并回发 `sub=15 op=10` ACK；nodeA 收到 `rx RETIRE_ACK applied rc=0`，执行 `RetireAck recovery retire`，guest `Retry error=0`，post-retire acquire 被 retired state 拒绝。
31. 最终目标中的 GSVA-specific coherence 仍未完整完成；当前已经有 GSVA route/coherence 模块、ARM MMU default GSVA path、manager descriptor CLI/peer descriptor distribution、manager-distributed descriptor import cleanup + retire、manager RetireAck-before-cleanup、descriptor-driven import、2/4 节点 acquire token 校验、2/4 节点 ACK-gated route-local token rotation、manager-distributed token revoke + holder ACK + TLB flush、event retire tombstone、四节点 writer invalidation、四节点 retire-while-shared、stale remap rejection、read-only write denial、higher epoch reuse、pending timeout 终态、timeout TLB flush、timeout query、2/4/8 节点 InvAck recovery、两节点 manager-distributed GSVA coherence recovery、两节点 active UB Link remote invalidate/ACK、两节点 active UB Link remote writeback/ACK 和两节点 active UB Link remote retire/ACK；剩余主要是 downgrade/fence/token revoke 的 active UB Link 化，以及 4/8 节点 manager-distributed recovery 验证。
...
5. Lifecycle transaction
   ...
   - 下一步是把 QEMU 侧 remote downgrade/fence/token revoke 从同步模拟推进到主动 UB Link 消息事务。
...
```text
已完成一个稳定的 GVA/GSVA 地址、GSVA segment descriptor ABI、manager descriptor CLI/peer descriptor distribution、manager-distributed descriptor import cleanup + retire、manager RetireAck-before-cleanup、descriptor-driven import、GSVA token acquire/ACK-gated route-local rotation、manager-distributed token revoke + holder ACK、manager-distributed coherence recovery、event retire tombstone、四节点 writer invalidation、四节点 retire-while-shared、stale epoch remap rejection、higher epoch reuse、active UB Link remote invalidate/writeback/retire、ARM MMU default GSVA path 与 OBMM MESI 数据层阶段。
下一阶段应从“能跑”转向“语义收敛”：把剩余 QEMU GSVA coherence op 从本地同步状态机推进到主动 UB Link remote downgrade/fence/token revoke 事务。
```
